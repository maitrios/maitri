#!/bin/bash
# Build maitri packages from mounted source (/maitri-source + /maitri-pkgs)
# and place the resulting .pkg.tar.zst files in the offline mirror.

set -e

offline_mirror_dir="$1"
if [[ -z $offline_mirror_dir ]]; then
  echo "Usage: build-maitri-packages.sh <offline-mirror-dir>" >&2
  exit 1
fi

if [[ ! -d /maitri-source ]]; then
  echo "ERROR: /maitri-source not mounted (pass --local-source or set MAITRI_SOURCE_PATH)" >&2
  exit 1
fi
if [[ ! -d /maitri-pkgs ]]; then
  echo "ERROR: /maitri-pkgs not mounted (set MAITRI_PKGS_PATH or place ../maitri-pkgs)" >&2
  exit 1
fi

work_dir=/tmp/maitri-pkg-build
rm -rf "$work_dir"
mkdir -p "$work_dir"

if ! id builder &>/dev/null; then
  useradd -m -s /bin/bash builder
fi
echo 'builder ALL=(ALL) NOPASSWD: /usr/bin/pacman' > /etc/sudoers.d/99-maitri-pkg-builder
chmod 440 /etc/sudoers.d/99-maitri-pkg-builder
chown builder:builder "$work_dir"

pacman -Sy --noconfirm

: "${MAITRI_RUNTIME_PACKAGE:=maitri-dev}"
: "${MAITRI_SETTINGS_PACKAGE:=maitri-settings-dev}"
: "${MAITRI_NVIM_PACKAGE:=maitri-nvim}"

packages=(
  "$MAITRI_SETTINGS_PACKAGE"
  "$MAITRI_RUNTIME_PACKAGE"
  "$MAITRI_NVIM_PACKAGE"
)

# Local-source packages must replace every cached build of the same package,
# even when the checkout's generated pkgver sorts below a published build.
# Otherwise the later generic cache pruning can silently keep edge instead.
for pkg in "${packages[@]}"; do
  rm -f "$offline_mirror_dir/$pkg-"*.pkg.tar.*
done

for pkg in "${packages[@]}"; do
  echo "----------------------------------------"
  echo "Building $pkg"
  echo "----------------------------------------"
  pkg_work="$work_dir/$pkg"
  if [[ ! -d "/maitri-pkgs/pkgbuilds/$pkg" ]]; then
    echo "ERROR: package source not found: /maitri-pkgs/pkgbuilds/$pkg" >&2
    exit 1
  fi
  cp -a "/maitri-pkgs/pkgbuilds/$pkg" "$pkg_work"
  chown -R builder:builder "$pkg_work"

  su builder -c "
    cd '$pkg_work' &&
    PKGDEST='$work_dir' \
    MAITRI_SRC=/maitri-source \
    makepkg --noconfirm --skippgpcheck --skipchecksums --nodeps -f
  "
done

mkdir -p "$offline_mirror_dir"
for package_file in "$work_dir"/*.pkg.tar.zst; do
  destination="$offline_mirror_dir/$(basename "$package_file")"

  # A cached signature belongs to the previously downloaded or locally built
  # package. Keeping it beside a newly built package makes pacman reject the
  # otherwise valid local-source build.
  rm -f "$destination" "$destination.sig"
  mv "$package_file" "$destination"
done

echo
echo "Built maitri packages, placed in $offline_mirror_dir:"
ls "$offline_mirror_dir"/maitri*.pkg.tar.zst | sed 's|^|  |'
