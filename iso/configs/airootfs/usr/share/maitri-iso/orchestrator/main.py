...
MAITRI_INSTALL_* environment variables before Python starts. This keeps
archinstall's import-time CLI parsing from seeing maitri-specific flags.
"""
...
def build_phases(ctx: InstallContext):
    ...
    and the partition details maitri needs for boot/fstab generation.
    """
    ...
    return [
        ...
        ("Installing Arch + maitri",  arch_install_system),
...
def main() -> int:
    ...
    info(f"Installing maitri for {who} → {ctx.target}")
