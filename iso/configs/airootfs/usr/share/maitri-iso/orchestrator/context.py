...
class InstallContext:
    ...
    maitri_install: dict[str, Any]
    defer_provisioning: bool = False

    target: Path = Path("/mnt")
    maitri_path: Path = Path("/usr/share/maitri")
    state_dir: Path = Path("/run/maitri-install")
    log_path: Path = Path("/var/log/maitri-install.log")
    target_log_path: Path = Path("/mnt/var/log/maitri-install.log")
    ...
    def from_env(cls) -> "InstallContext":
        config_str = os.environ.get("MAITRI_INSTALL_CONFIG")
        creds_str = os.environ.get("MAITRI_INSTALL_CREDS")
        if not config_str or not creds_str:
            raise RuntimeError("MAITRI_INSTALL_CONFIG and MAITRI_INSTALL_CREDS must be set")

        config_path = Path(config_str)
        creds_path = Path(creds_str)
        user_configuration = json.loads(config_path.read_text())
        maitri_install = user_configuration.get("maitri_install") or _default_maitri_install(user_configuration)

        # Deferred provisioning: the whole system installs but user creation is deferred to
        # first boot. Selected by the configurator (maitri_install.defer_provisioning) or by
        # an `defer-provisioning` marker file on an autoinstall drive, which also replaces the
        # user_credentials.json requirement.
        defer_provisioning_marker = _optional_path(os.environ.get("MAITRI_INSTALL_DEFER_PROVISIONING_FILE"))
        defer_provisioning = bool(maitri_install.get("defer_provisioning")) or defer_provisioning_marker is not None
        maitri_install["defer_provisioning"] = defer_provisioning
        ...
        arch_configuration.pop("maitri_install", None)
        ...
        state_dir = Path(os.environ.get("MAITRI_INSTALL_STATE_DIR", "/run/maitri-install"))
        ...
        ctx = cls(
            ...
            full_name=_read_text(os.environ.get("MAITRI_INSTALL_FULL_NAME_FILE")),
            email=_read_text(os.environ.get("MAITRI_INSTALL_EMAIL_FILE")),
            encrypt=_read_text(os.environ.get("MAITRI_INSTALL_ENCRYPT_FILE")).lower() in ("true", "yes", "1"),
            authorized_keys_path=_optional_path(os.environ.get("MAITRI_INSTALL_AUTHORIZED_KEYS_FILE")),
            tailscale_authkey_path=_optional_path(os.environ.get("MAITRI_INSTALL_TAILSCALE_AUTHKEY_FILE")),
            user_configuration=user_configuration,
            user_credentials=user_credentials,
            arch_config_path=arch_config_path,
            maitri_install=maitri_install,
            defer_provisioning=defer_provisioning,
            state_dir=state_dir,
        )
        disk_config = user_configuration.get("disk_config", {})
        target_mount = maitri_install.get("target_mount") or disk_config.get("mountpoint")
        if target_mount:
    ...
    @property
    def mode(self) -> str:
        if mode := self.maitri_install.get("mode"):
            return mode
...
def _default_maitri_install(user_configuration: dict) -> dict[str, Any]:
    disk_config = user_configuration.get("disk_config", {})
