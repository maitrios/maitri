...
Phase ordering (full-disk and protected/pre-mounted):
    ...
    arch_install_system    → one archinstall flow for partition/mount-or-use,
                             base install, early maitri packages, Limine setup,
                             useradd, runtime maitri packages, fstab
    configure_hibernation  → root-owned swap/resume drop-ins
    run_system_finalizer   → arch-chroot root maitri-apply-system, including Snapper
    finalize_limine_boot   → final Limine config/UKI build after hardware drop-ins
    run_chroot_finalizer   → arch-chroot -u user maitri-provision-user
...
# names explicitly instead of relying on provides=maitri resolution.
def _iso_ref() -> str:
    if ref := os.environ.get("MAITRI_ISO_REF"):
        return ref.strip()

    ref_file = Path("/root/maitri_iso_ref")
    if ref_file.exists():
        try:
...
def _default_package_targets() -> dict[str, str]:
    if _iso_ref() in {"dev", "local"}:
        return {
            "runtime": "maitri-dev",
            "settings": "maitri-settings-dev",
            "nvim": "maitri-nvim",
        }

    return {
        "runtime": "maitri",
        "settings": "maitri-settings",
        "nvim": "maitri-nvim",
    }
...
def _package_targets() -> dict[str, str]:
    ...
    targets_file = Path("/usr/share/maitri-iso/package-targets")
    if targets_file.exists():
        try:
            for raw in targets_file.read_text().splitlines():
                ...
                match key.strip():
                    case "MAITRI_RUNTIME_PACKAGE":
                        targets["runtime"] = value
                    case "MAITRI_SETTINGS_PACKAGE":
                        targets["settings"] = value
                    case "MAITRI_NVIM_PACKAGE":
                        targets["nvim"] = value
        except OSError:
            pass

    env_to_key = {
        "MAITRI_RUNTIME_PACKAGE": "runtime",
        "MAITRI_SETTINGS_PACKAGE": "settings",
        "MAITRI_NVIM_PACKAGE": "nvim",
    }
...
def _maitri_runtime_package() -> str:
    return _package_targets()["runtime"]


def _maitri_settings_package() -> str:
    return _package_targets()["settings"]


def _maitri_nvim_package() -> str:
    return _package_targets()["nvim"]


# Packages installed BEFORE useradd. The selected maitri-settings package and
# maitri-nvim populate /etc/skel so the user's home gets seeded correctly, and
# maitri-settings also ships the limine/snapper configs. Target-side setup
# commands are installed later by the selected maitri runtime package and
# executed in chroot.
...
# Install LuaRocks before maitri-nvim pulls in lua51-lpeg. Arch's lua-luarocks
...
def _early_bootstrap_packages() -> list[str]:
    return [*EARLY_BOOTSTRAP_BASE_PACKAGES, _maitri_settings_package()]


def _early_user_seed_packages() -> list[str]:
    return [_maitri_nvim_package()]
...
#
# archinstall is patched in the wrapper (maitri-iso-install) BEFORE Python
...
def prepare_live(ctx: InstallContext) -> None:
    if ctx.is_protected:
        ...
        if disk:
            ...
            subprocess.run(["maitri-iso-cleanup-disk", disk], check=True)
...
# Reorders guided.py's perform_installation() so early maitri packages install
# before user creation and before our maitri-owned Limine setup copies files
...
def arch_install_system(ctx: InstallContext) -> None:
    ...
    a pre-mounted target, and maitri derives boot/fstab details from that same
    input.
    """
    ...
    with arch.open_installer(config, ctx.target, silent=True) as installer:
        ...
        try:
            ...
            info("› installing maitri runtime + maitri-base.packages")
...
def _configure_limine_boot(ctx: InstallContext, installer, config) -> None:
    ...
    if not arch.is_limine(config):
        raise RuntimeError("maitri installs only support Limine bootloader setup")

    info("› installing bootloader (Limine)")
    if arch.is_pre_mount(config):
        _install_pre_mounted_limine(ctx)
    else:
        _install_limine_maitri(ctx, installer, config)
...
def _install_limine_maitri(ctx: InstallContext, installer, config) -> None:
...
def _install_pre_mounted_limine(ctx: InstallContext) -> None:
    ...
    if not esp_device:
        raise RuntimeError("maitri_install.storage.esp_device missing")
...
def _write_limine_pacman_hook(target: Path, hook_command: str) -> None:
    hook_contents = textwrap.dedent(
        ...
        [Action]
        Description = Deploying maitri Limine after upgrade...
    ...
    (hooks_dir / "99-maitri-limine.hook").write_text(hook_contents)
...
def _limine_template(ctx: InstallContext, filename: str) -> Path:
    candidates = [
        ctx.target / "usr" / "share" / "maitri" / "install" / "assets" / "limine" / filename,
        ctx.target / "usr" / "share" / "maitri" / "default" / "limine" / filename,
        ctx.maitri_path / "install" / "assets" / "limine" / filename,
        ctx.maitri_path / "default" / "limine" / filename,
    ]
...
# installed kernel. maitri-apply-system's hardware scripts routinely install
...
def _drop_archinstall_zram_conf(ctx: InstallContext) -> None:
    ...
    maitri-settings ships the tuning as a vendor drop-in at
    /usr/lib/systemd/zram-generator.conf.d/90-maitri.conf, which outranks the
...
def _install_early_packages(installer) -> None:
    ...
    info(f"› installing early maitri packages: {', '.join(bootstrap_packages)}")
...
def _mount_offline_package_cache(ctx: InstallContext) -> None:
    ...
    """
    source = Path("/var/cache/maitri/mirror/offline")
...
def _mask_mkinitcpio_pacman_hooks(
    ...
    for name in names:
        path = hooks_dir / name
        backup = hooks_dir / f"{name}.maitri-backup"
...
def _runtime_package_list(ctx: InstallContext) -> list[str]:
    """Selected maitri runtime package + every package in the ISO-bundled
    base package list that isn't already installed early."""
    base_pkgs_file = Path("/usr/share/maitri-iso/maitri-base.packages")
    pkgs = [_maitri_runtime_package()]
    already_installed = set(_early_packages()) | {
        _maitri_runtime_package(),
        _maitri_settings_package(),
        _maitri_nvim_package(),
        "maitri",
        "maitri-settings",
        "maitri-nvim",
    }
...
# ─────────────────────────────────────────────────────────────────────────────
# Install intent helpers: normalize the maitri-specific part of the
...
def _boot_intent(ctx: InstallContext) -> dict:
    boot = dict(ctx.maitri_install.get("boot") or {})
...
def _storage_intent(ctx: InstallContext) -> dict:
    return dict(ctx.maitri_install.get("storage") or {})
...
def verify_protected_mounts(ctx: InstallContext) -> None:
    ...
    for key in ("esp_device", "root_device"):
        ...
        if not device:
            raise RuntimeError(f"protected mode: maitri_install.storage.{key} missing")
...
def _btrfs_root_device(ctx: InstallContext) -> str:
    ...
    if storage.get("luks_uuid"):
        return storage.get("root_mapper") or "/dev/mapper/maitri_root"
...
def _write_pre_mounted_fstab(ctx: InstallContext) -> None:
    ...
    lines = [
        "# /etc/fstab — generated by maitri ISO",
...
def _write_pre_mounted_crypttab(ctx: InstallContext) -> None:
    ...
    crypttab.write_text(f"maitri_root  UUID={luks_uuid}  none  luks,discard\n")


def _build_pre_mounted_cmdline(ctx: InstallContext, btrfs_uuid: str) -> str:
    storage = _storage_intent(ctx)
    if storage.get("luks_uuid"):
        root_mapper = storage.get("root_mapper") or "/dev/mapper/maitri_root"
        return (
            f"cryptdevice=UUID={storage['luks_uuid']}:maitri_root "
...
def configure_hibernation(ctx: InstallContext) -> None:
    ...
    """
    setup = ctx.target / "usr" / "bin" / "maitri-hibernation-setup"
    if not setup.exists():
        _debug_log(ctx, "skipping hibernation: /usr/bin/maitri-hibernation-setup is not installed")
        return
    ...
    subprocess.run([
        ...
        "MAITRI_INSTALL_LOG_FILE=/var/log/maitri-install.log",
        "/usr/bin/maitri-hibernation-setup", "--force", "--no-rebuild",
    ], check=True)


def _install_debug_enabled() -> bool:
    return os.environ.get("MAITRI_INSTALL_DEBUG") == "1" or Path("/usr/share/maitri-iso/install-debug").exists()
...
#  3. arch-chroot as root → maitri-apply-system --first-install
#  4. arch-chroot as user → maitri-provision-user --first-install
# ─────────────────────────────────────────────────────────────────────────────
...
def _prepare_target_setup(ctx: InstallContext) -> None:
    ...
    bind_mounts = [
        ("/var/cache/maitri/mirror/offline", "/var/cache/maitri/mirror/offline"),
...
def _ensure_finalizer_log_started(ctx: InstallContext) -> tuple[str, int]:
    if "maitri_start_time" not in ctx.state:
        ctx.state["maitri_start_epoch"] = int(time.time())
        ctx.state["maitri_start_time"] = time.strftime("%Y-%m-%d %H:%M:%S")

    ctx.log_path.parent.mkdir(parents=True, exist_ok=True)
    ctx.log_path.touch(exist_ok=True)
    ctx.log_path.chmod(0o666)

    if not ctx.state.get("maitri_finalizer_header_written"):
        with ctx.log_path.open("a", encoding="utf-8") as log:
            log.write(f"=== maitri Target Setup Started: {ctx.state['maitri_start_time']} ===\n")
        ctx.state["maitri_finalizer_header_written"] = True

    return ctx.state["maitri_start_time"], ctx.state["maitri_start_epoch"]
...
def _run_target_setup_command(ctx: InstallContext, cmd: list[str], *, user: str | None = None) -> None:
    ...
    maitri_start_time, maitri_start_epoch = _ensure_finalizer_log_started(ctx)

    target_log = ctx.target / "var" / "log" / "maitri-install.log"
    ...
    mirror_channel = _read_maitri_mirror()
    env_extras = [
        "MAITRI_PATH=/usr/share/maitri",
        "MAITRI_INSTALL=/usr/share/maitri/install",
        f"MAITRI_INSTALL_USER={ctx.username}",
        f"MAITRI_START_TIME={maitri_start_time}",
        f"MAITRI_START_EPOCH={maitri_start_epoch}",
        f"MAITRI_USER_NAME={ctx.full_name}",
        f"MAITRI_USER_EMAIL={ctx.email}",
        f"MAITRI_MIRROR={mirror_channel}",
        f"MAITRI_ISO_REF={_iso_ref()}",
        f"MAITRI_RUNTIME_PACKAGE={_maitri_runtime_package()}",
        f"MAITRI_SETTINGS_PACKAGE={_maitri_settings_package()}",
        f"MAITRI_NVIM_PACKAGE={_maitri_nvim_package()}",
        "MAITRI_INSTALL_LOG_FILE=/var/log/maitri-install.log",
        "MAITRI_LOG_TO_STDOUT=1",
    ]
    if _install_debug_enabled():
        env_extras.append("MAITRI_INSTALL_DEBUG=1")
...
def run_system_finalizer(ctx: InstallContext) -> None:
    if ctx.defer_provisioning:
        cmd = ["/usr/bin/maitri-apply-system", "--defer-provisioning", "--first-install"]
    else:
        cmd = ["/usr/bin/maitri-apply-system", "--install-user", ctx.username, "--first-install"]
...
# setup (maitri-provision-owner) and factory reset (maitri-system-factory-reset) consume.
#
# Every install stashes the bundled Node tarball in /var/lib/maitri/provisioning/
...
PROVISION_STATE_DIR = "var/lib/maitri/provisioning"
PROVISION_KEYFILE = "etc/maitri/provisioning.key"
...
def stage_provisioning_state(ctx: InstallContext) -> None:
    ...
    service_src = ctx.target / "usr/share/maitri/install/provisioning/maitri-provision-owner.service"
    setup_bin = ctx.target / "usr/bin/maitri-provision-owner"
    if not service_src.exists() or not setup_bin.exists():
        raise RuntimeError(
            "deferred-provisioning install requested, but the installed maitri runtime does not ship "
            "first-boot setup (maitri-provision-owner + install/provisioning/maitri-provision-owner.service). "
    ...
    unit_dst = ctx.target / "etc/systemd/system/maitri-provision-owner.service"
    ...
    link = wants_dir / "maitri-provision-owner.service"
    link.unlink(missing_ok=True)
    link.symlink_to("/etc/systemd/system/maitri-provision-owner.service")
...
def _stage_provisioning_luks_unlock(ctx: InstallContext, provisioning_dir) -> None:
    ...
    cmdline_dropin = ctx.target / "etc/limine-entry-tool.d/99-maitri-provisioning-unlock.conf"
    cmdline_dropin.parent.mkdir(parents=True, exist_ok=True)
    cmdline_dropin.write_text(
        'KERNEL_CMDLINE[default]+=" cryptkey=rootfs:/etc/maitri/provisioning.key"\n'
    )

    files_dropin = ctx.target / "etc/mkinitcpio.conf.d/99-maitri-provisioning-key.conf"
    files_dropin.parent.mkdir(parents=True, exist_ok=True)
    files_dropin.write_text("FILES+=(/etc/maitri/provisioning.key)\n")
...
def finalize_limine_boot(ctx: InstallContext) -> None:
    ...
    if "maitri" not in limine_conf.read_text():
        raise RuntimeError(f"{limine_conf} has no maitri entry")
...
def run_chroot_finalizer(ctx: InstallContext) -> None:
    ...
    _run_target_setup_command(
        ctx,
        ["/usr/bin/maitri-provision-user", "--force", "--first-install"],
        user=ctx.username,
    )
...
def _read_maitri_mirror() -> str:
    p = Path("/root/maitri_mirror")
    return p.read_text().strip() if p.exists() else "stable"


# ─────────────────────────────────────────────────────────────────────────────
# configure_login: seed SDDM's last user/session for the password-only maitri
...
def configure_login(ctx: InstallContext) -> None:
    ...
    (sddm_dir / "99-maitri-login.conf").write_text(
        "[Theme]\nCurrent=maitri\n\n"
    ...
    if ctx.encrypt and not ctx.defer_provisioning:
        ...
        # deferred-provisioning installs have no user yet; maitri-provision-owner writes autologin
...
# keys an autoinstall drive supplied. A stock maitri install ships openssh but
...
def configure_ssh_access(ctx: InstallContext) -> None:
    ...
    if ctx.defer_provisioning:
        ...
        # maitri-provision-owner to install once first boot creates the owner, and
...
[Service]
Type=simple
ExecStart=/usr/bin/sh -c 'until tailscale up --auth-key file:{TAILSCALE_AUTHKEY_TARGET}; do sleep 15; done; rm -f {TAILSCALE_AUTHKEY_TARGET}; systemctl disable maitri-tailscale-join.service'
...
def configure_tailscale(ctx: InstallContext) -> None:
    ...
    unit = ctx.target / "etc" / "systemd" / "system" / "maitri-tailscale-join.service"
    unit.parent.mkdir(parents=True, exist_ok=True)
    unit.write_text(TAILSCALE_JOIN_UNIT)
    subprocess.run(
        ["arch-chroot", str(ctx.target), "systemctl", "enable",
         "tailscaled.service", "maitri-tailscale-join.service"],
        check=True,
    )
...
def validate_boot(ctx: InstallContext) -> None:
    ...
    if "maitri" not in limine_conf_text:
        raise RuntimeError(f"{limine_conf} has no maitri entry")
    ...
    uki_prefix = _limine_setting(config_text, "CUSTOM_UKI_NAME", "maitri") or "maitri"
    ...
    if arch.has_uefi():
        ...
        # Hardware packages (maitri-hw-intel-ptl, …) can swap the kernel out
...
def _validate_provisioning_state(ctx: InstallContext) -> None:
    ...
    link = ctx.target / "etc/systemd/system/multi-user.target.wants/maitri-provision-owner.service"
    if not link.is_symlink():
        raise RuntimeError("deferred-provisioning install but maitri-provision-owner.service is not enabled")
...
def _assert_boot_hooks_restored(ctx: InstallContext) -> None:
    ...
    for name in TARGET_DEFERRED_BOOT_HOOKS:
        ...
        # limine-mkinitcpio-hook is a hard dependency of the maitri runtime
...
# Taken at the end of every install, it is what makes maitri-system-factory-reset a
# true factory reset.
# ─────────────────────────────────────────────────────────────────────────────
...
FACTORY_SCRUB_PATHS = (
    "var/lib/maitri/provisioning/authorized_keys",
    "var/lib/maitri/provisioning/luks-key",
    "etc/maitri/provisioning.key",
    "etc/limine-entry-tool.d/99-maitri-provisioning-unlock.conf",
    "etc/mkinitcpio.conf.d/99-maitri-provisioning-key.conf",
    "etc/tailscale/authkey",
    "etc/systemd/system/maitri-tailscale-join.service",
    "etc/systemd/system/multi-user.target.wants/maitri-tailscale-join.service",
)
...
def cleanup_protected_state(ctx: InstallContext) -> None:
    ...
    if Path("/dev/mapper/maitri_root").exists():
        subprocess.run(
            ["cryptsetup", "close", "maitri_root"],
            check=False,
            capture_output=True,
        )
