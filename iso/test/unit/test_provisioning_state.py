...
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "configs/airootfs/usr/share/maitri-iso"))
...
class ContextDeferProvisioningTest(unittest.TestCase):
    def setUp(self):
        ...
        self.env = {
            "MAITRI_INSTALL_CONFIG": str(self.dir / "user_configuration.json"),
            "MAITRI_INSTALL_CREDS": str(self.dir / "user_credentials.json"),
            "MAITRI_INSTALL_STATE_DIR": str(self.state_dir),
        }
    ...
    def from_env(self, **extra_env):
        ...
        with mock.patch.dict(os.environ, env, clear=False):
            for key in ("MAITRI_INSTALL_DEFER_PROVISIONING_FILE",):
                if key not in env:
    ...
    def base_config(self, defer_provisioning=None, disk_encryption=None):
        config = {
            ...
            "maitri_install": {"mode": "full_disk", "target_mount": "/mnt"},
        }
        if defer_provisioning is not None:
            config["maitri_install"]["defer_provisioning"] = defer_provisioning
    ...
    def test_marker_file_arms_deferred_provisioning_without_credentials(self):
        ...
        marker.touch()
        ctx = self.from_env(MAITRI_INSTALL_DEFER_PROVISIONING_FILE=str(marker))
        self.assertTrue(ctx.defer_provisioning)
        self.assertTrue(ctx.maitri_install["defer_provisioning"])

    def test_absent_marker_file_is_not_deferred_provisioning(self):
        self.write_config(self.base_config())
        self.write_creds({"users": [{"username": "jeff"}]})
        ctx = self.from_env(MAITRI_INSTALL_DEFER_PROVISIONING_FILE=str(self.dir / "missing-defer_provisioning"))
...
def make_ctx(target, **overrides):
    defaults = dict(
        ...
        encrypt=False,
        username="",
        maitri_install={"mode": "full_disk", "defer_provisioning": True, "storage": {}},
...
class StageProvisioningStateTest(unittest.TestCase):
    ...
    def install_runtime_provisioning_support(self):
        service = self.target / "usr/share/maitri/install/provisioning/maitri-provision-owner.service"
        service.parent.mkdir(parents=True, exist_ok=True)
        service.write_text("[Unit]\n")
        setup_bin = self.target / "usr/bin/maitri-provision-owner"
        setup_bin.parent.mkdir(parents=True, exist_ok=True)
        setup_bin.write_text("#!/bin/bash\n")

    def provisioning_dir(self):
        return self.target / "var/lib/maitri/provisioning"
    ...
    def test_normal_install_stages_only_the_node_tarball(self):
        ...
        self.assertFalse((self.target / "etc/systemd/system/maitri-provision-owner.service").exists())
    ...
    def test_deferred_provisioning_arms_first_boot_setup(self):
        ...
        self.assertTrue((self.target / "etc/systemd/system/maitri-provision-owner.service").exists())
        link = self.target / "etc/systemd/system/multi-user.target.wants/maitri-provision-owner.service"
        self.assertTrue(link.is_symlink())
        self.assertEqual(
            os.readlink(link), "/etc/systemd/system/maitri-provision-owner.service"
        )
    ...
    def test_deferred_provisioning_encrypted_stages_luks_auto_unlock(self):
        ...
        keyfile = self.target / "etc/maitri/provisioning.key"
        self.assertEqual(keyfile.read_text(), "throwaway-secret")
        self.assertEqual(keyfile.stat().st_mode & 0o777, 0o600)

        cmdline = (self.target / "etc/limine-entry-tool.d/99-maitri-provisioning-unlock.conf").read_text()
        self.assertIn("cryptkey=rootfs:/etc/maitri/provisioning.key", cmdline)
        files = (self.target / "etc/mkinitcpio.conf.d/99-maitri-provisioning-key.conf").read_text()
        self.assertIn("FILES+=(/etc/maitri/provisioning.key)", files)

    def test_deferred_provisioning_pre_encrypted_without_passphrase_fails(self):
        self.install_runtime_provisioning_support()
        ctx = make_ctx(
            self.target,
            maitri_install={"mode": "protected", "defer_provisioning": True, "storage": {"luks_uuid": "abc"}},
        )
...
class ConfigureLoginDeferProvisioningTest(unittest.TestCase):
    ...
    def test_deferred_provisioning_login_leaves_user_state_to_first_boot(self):
        ...
        self.assertTrue((self.target / "etc/sddm.conf.d/99-maitri-login.conf").exists())
...
class ConfigureSshAccessDeferProvisioningTest(unittest.TestCase):
    ...
    def test_deferred_provisioning_stages_keys_for_first_boot(self):
        ...
        staged = self.target / "var/lib/maitri/provisioning/authorized_keys"
...
class CreateFactorySnapshotTest(unittest.TestCase):
    ...
    def test_snapshots_root_as_factory(self):
        ...
        self.assertIn(["mount", "-o", "subvolid=5", "/dev/mapper/maitri_root", top], self.calls)
        self.assertIn(
    ...
    def test_factory_snapshot_scrubs_provisioning_credentials(self):
        ...
        factory = top / "@factory"
        secrets = [
            factory / "var/lib/maitri/provisioning/authorized_keys",
            factory / "etc/tailscale/authkey",
            factory / "etc/maitri/provisioning.key",
        ]
        keep = factory / "var/lib/maitri/provisioning/groups"
