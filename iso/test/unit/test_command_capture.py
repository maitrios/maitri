...
EFIBOOTMGR_OUTPUT = (
    ...
    b"Boot0003* maitri\tHD(1,GPT,0d9c9d4f)/File(\\EFI\\limine\\BOOTX64.EFI)\n"
...
class CommandCaptureTest(unittest.TestCase):
    ...
    def test_undecodable_bytes_are_replaced_rather_than_raised(self):
        self.fake_command("maitri-not-utf8", b"before\x80after\n")
        self.assertEqual(capture(["maitri-not-utf8"]).stdout, "before�after\n")
    ...
    def test_efibootmgr_entries_parse_around_a_legacy_bbs_entry(self):
        ...
        self.assertEqual(phases_impl._find_label_entries(state["entries"], "maitri"), ["0003"])
    ...
    def test_a_mangled_btrfs_device_is_never_handed_to_mount(self):
        ...
        ctx = types.SimpleNamespace(target=Path("/mnt"), state_dir=Path("/run/maitri"))
