...
sys.path.insert(0, str(ROOT / "configs/airootfs/usr/share/maitri-iso"))

from orchestrator import keyboard as KEYBOARD  # noqa: E402

# The layout list lives in the maitri runtime now, shared verbatim with the
...
SETUP_FORM_CANDIDATES = (
    Path("/maitri-source/install/provisioning/setup-form.sh"),
    ROOT.parent / "maitri/install/provisioning/setup-form.sh",
    Path("/usr/share/maitri/install/provisioning/setup-form.sh"),
)
...
class KeyboardConfigurationTest(unittest.TestCase):
    ...
    def test_writes_keymap_and_xkb_settings_and_preserves_font(self):
        # XKBLAYOUT is load-bearing: maitri's detect-keyboard-layout.sh copies
    ...
    def test_all_configurator_keymaps_are_known_to_localectl(self):
        if SUPPORTED_KEYMAPS is None:
            self.skipTest("no maitri runtime checkout to read the layout list from")
