...
LIB="$ROOT/configs/airootfs/usr/share/maitri-iso/disk-partitioning.sh"
...
# shellcheck source=../configs/airootfs/usr/share/maitri-iso/disk-partitioning.sh
source "$LIB"
...
create_partition "$IMG" "$((1000 * MIB))" "$((1200 * MIB))" fat32 MAITRI_EFI
...
# up from EFI_END_B + 1.
create_partition "$IMG" "$((1201 * MIB))" "$((2000 * MIB))" btrfs MAITRI_ROOT
