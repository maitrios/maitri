#!/bin/bash
#
# Unit tests for maitri-install-diagnose-media and the failure screen that
...
DIAGNOSE="$ROOT/configs/airootfs/usr/local/bin/maitri-install-diagnose-media"
DASHBOARD="$ROOT/configs/airootfs/usr/local/bin/maitri-install-dashboard"
...
diagnose() {
  MAITRI_OFFLINE_MIRROR="$mirror" MAITRI_TARGET_PACKAGE_CACHE="$TARGET_CACHE" \
    "$DIAGNOSE" "$@"
}
...
{
  echo '#!/bin/bash'
  echo "MAITRI_OFFLINE_MIRROR='$mirror' MAITRI_TARGET_PACKAGE_CACHE='$TARGET_CACHE' exec '$DIAGNOSE' \"\$@\""
} >"$stubs/maitri-install-diagnose-media"
chmod +x "$stubs/maitri-install-diagnose-media"
...
maitri_share="$work/share"
mkdir -p "$maitri_share"
...
{"current_phase": "Installing Arch + maitri",
 "phases": [{"name": "Installing Arch + maitri", "status": "failed",
             "error": "Pacstrap failed. See /var/log/archinstall.log"}]}
STATE
  : >"$screen"
  script -qefc "stty rows 40 cols 120; PATH='$stubs:$PATH' MAITRI_PATH='$maitri_share' MAITRI_UI_INTERACTIVE=no MAITRI_UI_FAILURE_ACTION=exit MAITRI_FAILURE_TAIL_LOG='$install_log' '$DASHBOARD' '$install_log' '$state_file' -- bash -c 'exit 1'" \
    "$screen" >/dev/null 2>&1
}
...
visible_screen | grep -qF "maitri installation stopped" ||
