-- Window and layer rules for the maitri Quickshell surfaces. The
-- shell-wide bar / menu / popouts are layer-shell.

-- Keep the bar instant: no layer-shell fade/slide animation.
hl.layer_rule({ match = { namespace = "maitri-bar" }, no_anim = true, animation = "none" })
...
hl.layer_rule({ match = { namespace = "^(maitri-menu|maitri-image-selector|maitri-emojis|maitri-clipboard|maitri-keyboard-panel)$" }, no_anim = true, animation = "none" })

-- Dev gallery is the main shell workbench; open it maximized like
-- SUPER+ALT+F so component previews have the whole workspace.
o.window({ class = "^org.quickshell$", title = "^maitri shell – dev gallery$" }, { maximize = true })
