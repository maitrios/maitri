...
Hooks live in `~/.config/maitri/hooks/<name>.d/` — one directory per event,
holding any number of independent scripts. Install with
`maitri hook install <name> <script>` (copies the script in and makes it
executable). The runner also executes a flat `~/.config/maitri/hooks/<name>`
...
├── post-update.d/          # During `maitri update`, after system packages and migrations
├── pre-refresh-pacman.d/   # Before `maitri refresh pacman` re-syncs packages
