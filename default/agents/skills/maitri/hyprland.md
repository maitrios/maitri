...
maitri configures Hyprland in Lua. User files are loaded after maitri's
defaults, so overrides go here:

```
~/.config/hypr/
├── hyprland.lua       # Main config (loads maitri defaults, then user files)
...
- Use `maitri refresh hyprland` to reset the Lua config files to defaults

The two `.conf` files are read by separate processes, so `hyprctl` neither
applies nor validates them:
- `hyprsunset.conf` (night light): apply changes with `maitri restart hyprsunset`; reset with `maitri refresh hyprsunset`
...
Window rules go in `~/.config/hypr/hyprland.lua` or a required Lua module. Prefer maitri's `o.window(match, rules)` helper — see examples in `$MAITRI_PATH/default/hypr/windows.lua`.
