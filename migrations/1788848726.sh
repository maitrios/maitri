echo "Retire the stock user icon font missed by the Omarchy 4 upgrade"

legacy_font="$HOME/.local/share/fonts/maitri.ttf"

# The upgrader treated this as ~/.config/maitri.ttf and left the old family
...
if [[ -f $legacy_font && ! -L $legacy_font ]]; then
  ...
  if [[ ${legacy_hash%% *} == "e55e67119e82f56f92d90cbf54b7ccc1b2946b32c535a29370439d7ef5215966" ]]; then
    if [[ ! -f /usr/share/fonts/maitri/maitri.ttf ]]; then
      echo "Packaged maitri icon font is missing; keeping the legacy font." >&2
      exit 1
    fi
