...
# config, but maitri-settings ships maitri-defaults.conf with that parameter
...
# landed. Such a machine boots without any of maitri-defaults.conf's command
# line — including initramfs_async=0, so Plymouth still loses the LUKS prompt.

defaults_conf="${MAITRI_LIMINE_DEFAULTS_CONF:-/etc/limine-entry-tool.d/maitri-defaults.conf}"
running_cmdline="${MAITRI_RUNNING_CMDLINE:-/proc/cmdline}"
rebuild_marker="${MAITRI_LIMINE_REBUILD_MARKER:-/var/lib/maitri/migrations/1786482992}"
