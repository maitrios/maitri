echo "Repair legacy XCompose and remove vulnerable maitri 3 power udev rules"

xcompose="$HOME/.XCompose"
packaged_xcompose="$MAITRI_PATH/default/xcompose"
legacy_xcompose_pattern='^[[:space:]]*include[[:space:]]+"[^"]*/\.local/share/maitri/default/xcompose"[[:space:]]*$'

# maitri 3 pointed the user's compose file through the checkout compatibility
...
if [[ -f $xcompose ]] && grep -Eq "$legacy_xcompose_pattern" "$xcompose"; then
  ...
  xcompose_replacement=${xcompose_replacement//|/\\|}
  sed -i -E "s|^([[:space:]]*include[[:space:]]+\")[^\"]*/\\.local/share/maitri/default/xcompose\"[[:space:]]*$|\\1$xcompose_replacement\"|" "$xcompose"
  maitri-restart-xcompose >/dev/null 2>&1 || true
fi

rules_dir=/etc/udev/rules.d
reload_marker_prefix=/var/lib/maitri/migrations/1788102906-udev-reload-needed
...
# maitri 3 generated these two rules with an unquoted heredoc, so the installing
...
rule_runs_from_home() {
  ...
  while IFS= read -r line || [[ -n $line ]]; do
    ...
    while [[ $rest =~ $run_pattern ]]; do
      ...
      if [[ $command == *"/.local/share/maitri/bin/$binary"* ]]; then
        return 0
      fi
    done
  done <"$file"
...
mark_reload_needed() {
  if ! as_root "$install_command" -Dm644 /dev/null "$reload_needed_marker"; then
    echo "Administrator privileges are required to repair the vulnerable legacy udev rule. Ask an administrator to run maitri-migrate." >&2
    exit 1
  fi
}
...
rule_is_exact_generated() {
  ...
  case "$binary" in
    maitri-powerprofiles-set)
      # Initial userland helper: separate AC/battery units and arguments.
      prefix='SUBSYSTEM=="power_supply", ATTR{type}=="Mains", ATTR{online}=="0", RUN+="/usr/bin/systemd-run --no-block --collect --unit=maitri-power-profile-battery --property=After=power-profiles-daemon.service '
      suffix='/.local/share/maitri/bin/maitri-powerprofiles-set battery"'
      if [[ ${lines[0]} == "$prefix"*"$suffix" ]]; then
        home=${lines[0]#"$prefix"}
        home=${home%"$suffix"}
        expected='SUBSYSTEM=="power_supply", ATTR{type}=="Mains", ATTR{online}=="1", RUN+="/usr/bin/systemd-run --no-block --collect --unit=maitri-power-profile-ac --property=After=power-profiles-daemon.service '"$home"'/.local/share/maitri/bin/maitri-powerprofiles-set ac"'
        [[ $home == /* && $home != *'"'* && ${lines[1]} == "$expected" ]] && return 0
      fi

      # USB-C support: one fixed transient-unit name and no profile argument.
      prefix='SUBSYSTEM=="power_supply", ATTR{type}=="Mains", RUN+="/usr/bin/systemd-run --no-block --collect --unit=maitri-power-profile --property=After=power-profiles-daemon.service '
      suffix='/.local/share/maitri/bin/maitri-powerprofiles-set"'
      if [[ ${lines[0]} == "$prefix"*"$suffix" ]]; then
        home=${lines[0]#"$prefix"}
        home=${home%"$suffix"}
        expected='SUBSYSTEM=="power_supply", ATTR{type}=="USB", RUN+="/usr/bin/systemd-run --no-block --collect --unit=maitri-power-profile --property=After=power-profiles-daemon.service '"$home"'/.local/share/maitri/bin/maitri-powerprofiles-set"'
        [[ $home == /* && $home != *'"'* && ${lines[1]} == "$expected" ]] && return 0
      fi

      # The final maitri 3 revision dropped the fixed transient-unit name.
      ...
      if [[ ${lines[0]} == "$prefix"*"$suffix" ]]; then
        home=${lines[0]#"$prefix"}
        home=${home%"$suffix"}
        expected='SUBSYSTEM=="power_supply", ATTR{type}=="USB", RUN+="/usr/bin/systemd-run --no-block --collect --property=After=power-profiles-daemon.service '"$home"'/.local/share/maitri/bin/maitri-powerprofiles-set"'
        [[ $home == /* && $home != *'"'* && ${lines[1]} == "$expected" ]] && return 0
      fi
      ;;
    maitri-wifi-powersave)
      # Initial Wi-Fi helper: invoke the userland command directly.
      prefix='SUBSYSTEM=="power_supply", ATTR{type}=="Mains", ATTR{online}=="0", RUN+="'
      suffix='/.local/share/maitri/bin/maitri-wifi-powersave on"'
      if [[ ${lines[0]} == "$prefix"*"$suffix" ]]; then
        home=${lines[0]#"$prefix"}
        home=${home%"$suffix"}
        expected='SUBSYSTEM=="power_supply", ATTR{type}=="Mains", ATTR{online}=="1", RUN+="'"$home"'/.local/share/maitri/bin/maitri-wifi-powersave off"'
        [[ $home == /* && $home != *'"'* && ${lines[1]} == "$expected" ]] && return 0
      fi

      # Later revision deferred each change through its own transient unit.
      prefix='SUBSYSTEM=="power_supply", ATTR{type}=="Mains", ATTR{online}=="0", RUN+="/usr/bin/systemd-run --no-block --collect --unit=maitri-wifi-powersave-on '
      suffix='/.local/share/maitri/bin/maitri-wifi-powersave on"'
      if [[ ${lines[0]} == "$prefix"*"$suffix" ]]; then
        home=${lines[0]#"$prefix"}
        home=${home%"$suffix"}
        expected='SUBSYSTEM=="power_supply", ATTR{type}=="Mains", ATTR{online}=="1", RUN+="/usr/bin/systemd-run --no-block --collect --unit=maitri-wifi-powersave-off '"$home"'/.local/share/maitri/bin/maitri-wifi-powersave off"'
        [[ $home == /* && $home != *'"'* && ${lines[1]} == "$expected" ]] && return 0
      fi
      ;;
  esac
...
finish_pending_reload() {
  ...
  if [[ -e $udev_control ]] && ! as_root "$udevadm_command" control --reload 2>/dev/null; then
    echo "Could not reload udev after removing a vulnerable legacy rule. Ask an administrator to run maitri-migrate." >&2
    exit 1
  fi

  if ! as_root "$rm_command" -f "$reload_needed_marker"; then
    echo "Could not finish the legacy udev-rule repair. Ask an administrator to run maitri-migrate." >&2
    exit 1
  fi
}
...
quarantine_rule() {
  local rule_file="$1"
  local backup="$rule_file.maitri-disabled"
  local suffix=0
  ...
  while [[ -e $backup || -L $backup ]]; do
    ((++suffix))
    backup="$rule_file.maitri-disabled.$suffix"
  done

  mark_reload_needed
  if ! as_root "$mv_command" --no-clobber -- "$rule_file" "$backup"; then
    echo "Administrator privileges are required to quarantine the vulnerable legacy udev rule. Ask an administrator to run maitri-migrate." >&2
    exit 1
  fi
  if [[ -e $rule_file || -L $rule_file ]]; then
    echo "Could not quarantine the vulnerable legacy udev rule at $rule_file. Ask an administrator to run maitri-migrate." >&2
    exit 1
  fi
...
if [[ -d $rules_dir && ! -x $rules_dir ]]; then
  echo "Could not inspect legacy udev rules under $rules_dir. Ask an administrator to run maitri-migrate." >&2
  exit 1
fi

for legacy_rule in "99-power-profile.rules:maitri-powerprofiles-set" "99-wifi-powersave.rules:maitri-wifi-powersave"; do
  ...
  if [[ -f $rule_file && ! -r $rule_file ]]; then
    echo "Could not inspect the legacy udev rule at $rule_file. Ask an administrator to run maitri-migrate." >&2
    exit 1
  fi
  ...
  if [[ -f $rule_file ]] && rule_is_exact_generated "$rule_file" "$binary"; then
    ...
    if ! as_root "$rm_command" -f "$rule_file"; then
      echo "Administrator privileges are required to remove the vulnerable legacy udev rule. Ask an administrator to run maitri-migrate." >&2
      exit 1
    fi
