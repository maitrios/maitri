...
current_absolute_target="$HOME/.local/state/maitri/current/theme/neovim.lua"
...
# one ("../../../../.config/maitri/current/theme/neovim.lua"), leaving those
# links dangling. Match the shared suffix instead so every spelling is repaired.
case "$target" in
  */maitri/current/theme/neovim.lua) ;;
  *) exit 0 ;;
esac
