echo "Install the maitri kernel and make it the first Limine boot entry"

# linux-maitri is an x86_64 kernel. T2 Macs must keep their specialized kernel,
# including when other kernels are installed or the running T2 package is gone.
[[ $(uname -m) == "x86_64" ]] || exit 0
running_kernel=$(uname -r)
if maitri-pkg-present linux-t2 || [[ ${running_kernel,,} == *-t2* ]]; then
  exit 0
fi

limine_conf="${MAITRI_KERNEL_LIMINE_CONF:-/etc/default/limine}"
rebuild_marker="${MAITRI_KERNEL_REBUILD_MARKER:-/var/lib/maitri/migrations/1789325478}"
kernel="linux-maitri"
...
maitri-pkg-add "$kernel" "$kernel-headers"
...
# Set the exact kernel first: linux-maitri-* only matches its older variants.
...
printf '\n%s\n' 'BOOT_ORDER="linux-maitri, linux-maitri-*, *, *fallback, Snapshots"' | sudo tee -a "$limine_conf" >/dev/null
...
if ! sudo limine-entry-tool --tree | grep -E "(^|[^[:alnum:]_-])$kernel([^[:alnum:]_-]|$)" >/dev/null; then
  echo "The maitri kernel has no Limine boot entry; rerun maitri-migrate after fixing the boot image build." >&2
  exit 1
fi
