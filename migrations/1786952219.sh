echo "Switch mise to the mise-bin package from the maitri repo"
...
# maitri-zsh and maitri-fish, which depend on it; the provides is what keeps
# them satisfied when mise-bin lands in the same transaction that drops mise.
#
# maitri-pkg-add cannot do that swap: pacman answers its own conflict question
...
if maitri-pkg-missing mise-bin; then
