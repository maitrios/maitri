...
# maitri-setup-security-fingerprint.
#
# The gate points at the fixed /usr/bin path the maitri package always
# provides, so it keeps working across package installs and dev-link (which
# overlays $MAITRI_PATH but leaves /usr/bin untouched). pam_exec needs a
# literal absolute path — it does not expand env vars.

gate="auth      [success=1 default=ignore] pam_exec.so quiet /usr/bin/maitri-hw-laptop-closed"

for pam in /etc/pam.d/sudo /etc/pam.d/polkit-1; do
  if [[ -f $pam ]] &&
    grep -q 'pam_fprintd\.so' "$pam" &&
    ! grep -q 'maitri-hw-laptop-closed' "$pam"; then
