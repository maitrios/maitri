...
# pacman-rc.conf shipped with [maitri] pointing at the edge repository, a
...
# maitri packages from edge. Repoint only a conf that still carries the
...
if grep -q "https://rc-mirror.omarchy.org/" /etc/pacman.d/mirrorlist &&
  ...
  echo "Switched the [maitri] repository to the rc channel to match this machine's rc mirror."
fi
