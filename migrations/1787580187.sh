...
# rewrite the host), so maitri no longer puts users in it by default. Bring
...
# it from prompting mid-update — maitri-update-restart handles the reboot once
# the whole update has finished.
if id -nG "$USER" | grep -qw docker; then
  MAITRI_DEFER_REBOOT=1 maitri-remove-security-sudoless-docker
fi
...
if [[ -f $dest ]]; then
  cp "$MAITRI_PATH/applications/Docker.desktop" "$dest"
fi
