echo "Move current maitri theme state to ~/.local/state"

legacy_current_dir="$HOME/.config/maitri/current"
...
replace_current_path() {
  ...
  replace_literal_in_file "$file" "$HOME/.config/maitri/current" "$HOME/.local/state/maitri/current"
  replace_literal_in_file "$file" "~/.config/maitri/current" "~/.local/state/maitri/current"
  replace_literal_in_file "$file" "../maitri/current" "../../.local/state/maitri/current"
}
...
relink_current_symlink() {
  ...
  case "$target" in
    ...
    "~/.config/maitri/current/"*)
      ...
      suffix=${target#"~/.config/maitri/current/"}
      ln -sfn "$current_state_dir/$suffix" "$link"
      ;;
  esac
}
...
for link in \
  ...
  "$HOME/.config/helix/themes/maitri.toml" \
  "$HOME/.vscode/extensions/maitri-theme/themes/maitri-color-theme.json" \
  "$HOME/.vscode-insiders/extensions/maitri-theme/themes/maitri-color-theme.json" \
  "$HOME/.vscode-oss/extensions/maitri-theme/themes/maitri-color-theme.json" \
  "$HOME/.cursor/extensions/maitri-theme/themes/maitri-color-theme.json"; do
