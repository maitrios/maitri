...
sleep_lock_unit="$user_config_home/systemd/user/maitri-sleep-lock.service"
sleep_lock_dropin_dir="$user_config_home/systemd/user/maitri-sleep-lock.service.d"
sleep_lock_dropin="$sleep_lock_dropin_dir/90-maitri-session-environment.conf"
...
if [[ -f $sleep_lock_unit ]]; then
  ...
  printf '%s\n' \
    ...
    'ConditionEnvironment=MAITRI_PATH' \
...
if ! error=$(systemctl --user show-environment 2>&1); then
  if [[ -S $user_manager_socket ]]; then
    ...
    echo "The pre-suspend lock repair will be retried by maitri-migrate."
    exit 1
  fi
  exit 0
fi
...
if [[ $graphical_state == "active" ]]; then
  if ! error=$(systemctl --user reset-failed maitri-sleep-lock.service 2>&1); then
    echo "Could not reset maitri-sleep-lock.service: $error"
    echo "The pre-suspend lock repair will be retried by maitri-migrate."
    exit 1
  elif ! error=$(systemctl --user restart maitri-sleep-lock.service 2>&1); then
    echo "Could not restart maitri-sleep-lock.service: $error"
    echo "The pre-suspend lock repair will be retried by maitri-migrate."
    exit 1
  elif ! sleep_lock_state=$(systemctl --user show --property=ActiveState --value maitri-sleep-lock.service 2>&1); then
    echo "Could not inspect maitri-sleep-lock.service: $sleep_lock_state"
    echo "The pre-suspend lock repair will be retried by maitri-migrate."
    exit 1
  elif [[ $sleep_lock_state != "active" ]]; then
    echo "maitri-sleep-lock.service did not stay active after restart."
    echo "The pre-suspend lock repair will be retried by maitri-migrate."
    exit 1
  fi
else
  ...
  if ! error=$(systemctl --user stop maitri-sleep-lock.service 2>&1); then
    echo "Could not stop stale maitri-sleep-lock.service: $error"
    echo "The pre-suspend lock repair will be retried by maitri-migrate."
    exit 1
  elif ! error=$(systemctl --user reset-failed maitri-sleep-lock.service 2>&1); then
    echo "Could not reset maitri-sleep-lock.service: $error"
    echo "The pre-suspend lock repair will be retried by maitri-migrate."
    exit 1
  fi
