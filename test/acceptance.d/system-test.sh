...
verify_kernel_headers() {
  local kernel=linux-maitri
  local release
  release=$(uname -r)
  maitri-pkg-present linux-t2 && kernel=linux-t2

  [[ $(cat "/usr/lib/modules/$release/pkgbase") == "$kernel" ]] ||
    fail "the installed system boots the supported kernel" "$release is not $kernel"
  maitri-pkg-present "$kernel-headers" || fail "kernel headers are installed" "$kernel-headers is missing"
