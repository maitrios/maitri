...
open_and_close() {
  ...
  if [[ -n $payload ]]; then
    maitri-shell shell summon "$plugin" "$payload" >/dev/null
  else
...
wtype -k Return
wait_until "emoji picker selection closes" 15 layer_absent "maitri-emojis"

# Seed two clipboard entries, search for the older one, and copy it back out.
clipboard_token="maitri acceptance clipboard $(date +%s)"
printf '%s' "$clipboard_token" | wl-copy
wait_until "clipboard history captures test text" 15 grep -Fq "$clipboard_token" "$HOME/.local/state/maitri/clipboard-history.json"
printf '%s' "clipboard decoy" | wl-copy
sleep 1

maitri-shell shell summon maitri.clipboard >/dev/null
wait_until "clipboard opens" 15 layer_present "maitri-clipboard"
wtype "$clipboard_token"
wait_until "clipboard search finds test text" 15 screen_contains "maitri acceptance clipboard"
screenshot "success-clipboard-search"
wtype -M shift -k Return -m shift
wait_until "clipboard selection closes" 15 layer_absent "maitri-clipboard"
...
maitri-shell shell summon maitri.menu '{"menu":"system"}' >/dev/null
wait_until "system menu opens" 15 layer_present "maitri-menu"
wait_until "system menu content is visible" 15 screen_contains "Shutdown"
screenshot "success-system-menu"
wtype -k Escape
wait_until "system menu closes" 15 layer_absent "maitri-menu"
...
launch_app "maitri-theme-bg-switcher"
wait_until "background selector opens" 30 layer_present "maitri-image-selector"
sleep 1
screenshot "success-background-selector"
wtype -k Escape
wait_until "background selector closes" 15 layer_absent "maitri-image-selector"

launch_app "maitri-theme-switcher"
wait_until "theme selector opens" 30 layer_present "maitri-image-selector"
sleep 1
screenshot "success-theme-selector"
wtype -k Escape
wait_until "theme selector closes" 15 layer_absent "maitri-image-selector"

# Walk the reminder flow through each input screen, but dismiss before it
# schedules a real timer in the test user's session.
maitri-shell shell summon maitri.reminders >/dev/null
wait_until "reminder flow opens" 15 layer_present "maitri-reminders"
...
wtype -k Escape
wait_until "reminder flow closes" 15 layer_absent "maitri-reminders"

# Render a real shell notification and clear it through the notification IPC.
maitri-shell notifications dismissAll >/dev/null
maitri-notification-send "Acceptance notification" "Shell notification rendering" --expire-time=15000
wait_until "notification popup opens" 15 layer_present "maitri-notifications"
...
maitri-shell notifications dismissAll >/dev/null
wait_until "notification popup closes" 15 layer_absent "maitri-notifications"
...
wait_until "apps menu opens" 15 layer_present "maitri-menu"
sleep 1
