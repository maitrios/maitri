...
chmod +x "$stub_bin/maitri-update-available" "$stub_bin/maitri-shell"

PATH="$stub_bin:$PATH" SHELL_CALLS="$shell_calls" UPDATE_AVAILABLE_STATUS=0 \
  "$ROOT/bin/maitri-update-status"
grep -Fx -- "-q maitri.system-update refresh" "$shell_calls" >/dev/null ||
...
PATH="$stub_bin:$PATH" SHELL_CALLS="$shell_calls" UPDATE_AVAILABLE_STATUS=1 \
  "$ROOT/bin/maitri-update-status"
grep -Fx -- "-q maitri.system-update clear" "$shell_calls" >/dev/null ||
