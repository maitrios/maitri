...
other_packages="$ROOT/install/maitri-other.packages"
...
cat >"$stub_bin/maitri-pkg-present" <<'SH'
#!/bin/bash
...
printf 'maitri-pkg-drop' >>"$TEST_LOG"
printf '\t%s' "$@" >>"$TEST_LOG"
printf '\n' >>"$TEST_LOG"
SH
...
PATH="$stub_bin:$PATH" \
  ...
  MAITRI_T2_LIMINE_CONF="$limine_conf" \
  MAITRI_T2_FAN_CONF="$fan_conf" \
  MAITRI_T2_RUNNING_CMDLINE="$running_cmdline" \
  MAITRI_T2_REPAIR_MARKER="$repair_marker" \
...
grep -Fq $'maitri-pkg-drop\ttiny-dfr' "$calls" ||
...
! grep -Eq $'^(sudo\t)?(sed|tee|systemctl|maitri-pkg-drop)(\t|$)' "$calls" ||
