...
jq empty "$ROOT/config/maitri/shell.json"
pass "default shell.json is valid JSON"

jq -e '.version == 1 and (.bar.layout.left | type == "array") and (.bar.layout.center | type == "array") and (.bar.layout.right | type == "array")' "$ROOT/config/maitri/shell.json" >/dev/null
...
jq -e '
  ...
  ($ids | index("maitri.weather")) as $weather |
  ($ids | index("maitri.system-update")) as $update |
  $weather != null and $update == $weather + 1
' "$ROOT/config/maitri/shell.json" >/dev/null
...
jq -e '
  any(.bar.layout.center[]; (.id // .) == "maitri.clock" and (.formatAlt // "") == "d MMMM \u0027W\u0027ww yyyy")
' "$ROOT/config/maitri/shell.json" >/dev/null
...
pkgs_candidates = [
  ...
  root.parent / "maitri/maitri-pkgs/pkgbuilds",
...
if pkgs_root is None:
  print("not ok - maitri-pkgs checkout found for PKGBUILD coverage", file=sys.stderr)
  print(
    "looked in:\n  " + "\n  ".join(str(path) for path in pkgs_candidates) +
    "\nset MAITRI_PKGS_PATH to the maitri-pkgs checkout",
    file=sys.stderr,
  )
  sys.exit(1)
settings_pkgbuild_path = pkgs_root / "maitri-settings/PKGBUILD"
maitri_pkgbuild_path = pkgs_root / "maitri/PKGBUILD"
if not settings_pkgbuild_path.exists():
  settings_pkgbuild_path = pkgs_root / "maitri-settings-dev/PKGBUILD"
if not maitri_pkgbuild_path.exists():
  maitri_pkgbuild_path = pkgs_root / "maitri-dev/PKGBUILD"
pkgbuild = settings_pkgbuild_path.read_text()
maitri_pkgbuild = maitri_pkgbuild_path.read_text()
errors = []
package_defaults = [
  ("default/uwsm/env.d/10-maitri", "/usr/share/uwsm/env.d/10-maitri", "uwsm/env"),
  ("default/uwsm/default", None, "uwsm/default"),
  ("default/environment.d/10-maitri-fcitx.conf", "/usr/lib/environment.d/10-maitri-fcitx.conf", "environment.d/fcitx.conf"),
  ("default/fontconfig/conf.avail/50-maitri.conf", "/usr/share/fontconfig/conf.avail/50-maitri.conf", "fontconfig/fonts.conf"),
  ...
  ("default/systemd/user/maitri-crash-watch.service", "/usr/lib/systemd/user/maitri-crash-watch.service", "systemd/user/maitri-crash-watch.service"),
  ("default/systemd/zram-generator.conf.d/90-maitri.conf", "/usr/lib/systemd/zram-generator.conf.d/90-maitri.conf", "systemd/zram-generator.conf.d/90-maitri.conf"),
  ("default/systemd/system/plocate-updatedb.service.d/10-maitri.conf", "/usr/lib/systemd/system/plocate-updatedb.service.d/10-maitri.conf", "systemd/system/plocate-updatedb.service.d/10-maitri.conf"),
  ("default/fonts/maitri/maitri.ttf", "/usr/share/fonts/maitri/maitri.ttf", "maitri.ttf"),
  ("default/snapper/root", "/etc/snapper/config-templates/maitri", "snapper/root"),
]
...
alpm_hooks = [
  ...
  "10-maitri-hyprland-reload-pause.hook",
  "90-maitri-hyprland-reload-resume.hook",
]
for hook in alpm_hooks:
  ...
  if not (root / source).exists():
    errors.append(f"missing package default source: {source}")
  if source not in maitri_pkgbuild or destination not in maitri_pkgbuild:
    errors.append(f"maitri PKGBUILD does not install {source} -> {destination}")
...
grep -F 'dofile((os.getenv("MAITRI_PATH") or "/usr/share/maitri") .. "/default/hypr/bootstrap.lua")' "$ROOT/config/hypr/hyprland.lua" >/dev/null
grep -F 'require("default.hypr.maitri")' "$ROOT/config/hypr/hyprland.lua" >/dev/null
...
package.loaded["maitri.current.theme.hyprland"] = true
package.loaded["unrelated.module"] = true

dofile(os.getenv("MAITRI_PATH") .. "/default/hypr/bootstrap.lua")

assert(package.loaded["default.hypr.maitri"] == nil)
assert(package.loaded["default.hypr.require_optional"] == nil)
assert(package.loaded["hypr.looknfeel"] == nil)
assert(package.loaded["maitri.current.theme.hyprland"] == nil)
assert(package.loaded["unrelated.module"] == true)
LUA
pass "Hyprland bootstrap reloads cached maitri config modules"

TMPDIR=$(mktemp -d)
mkdir -p "$TMPDIR/home/.config/maitri"

ipc_mock_bin="$TMPDIR/ipc-mock"
mkdir -p "$ipc_mock_bin"
cat >"$ipc_mock_bin/maitri-shell" <<'SH'
#!/bin/bash
set -euo pipefail

mkdir -p "$HOME/.local/state/maitri"
printf '%s\n' "$*" >>"$HOME/.local/state/maitri/shell-ipc-calls"
printf 'ok\n'
SH
chmod +x "$ipc_mock_bin/maitri-shell"
export PATH="$ipc_mock_bin:$PATH"

cat >"$TMPDIR/home/.config/maitri/shell.json" <<'JSON'
{
  "version": 1,
  "bar": {
    "layout": {
      "left": [{ "id": "maitri.menu" }, { "id": "maitri.workspaces" }, { "id": "maitri.active-window" }],
      "center": [{ "id": "maitri.clock" }, { "id": "maitri.weather" }, { "id": "maitri.system-update" }, { "id": "maitri.tailscale" }],
      "right": [{ "id": "maitri.tray" }, { "id": "maitri.microphone" }, { "id": "maitri.bluetooth" }]
    }
  },
  "plugins": []
}
...
mkdir -p "$TMPDIR/home/.config/maitri/plugins/local.demo-bar"
cat >"$TMPDIR/home/.config/maitri/plugins/local.demo-bar/manifest.json" <<'JSON'
{
  "schemaVersion": 1,
  "id": "local.demo-bar",
  "name": "Demo bar",
...
JSON
touch "$TMPDIR/home/.config/maitri/plugins/local.demo-bar/Bar.qml"

if HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" maitri-bar use local.nonexistent-bar 2>/dev/null; then
  fail "bar use accepted an unknown bar option"
fi
pass "bar use rejects an unknown bar option"

HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" maitri-bar use local.demo-bar
jq -e '.bar.id == "local.demo-bar"' "$TMPDIR/home/.config/maitri/shell.json" >/dev/null
pass "shell config selects a bar option"

HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" maitri-bar reset
jq -e '.bar.id == null' "$TMPDIR/home/.config/maitri/shell.json" >/dev/null
pass "shell config resets to built-in bar option"

HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" maitri-bar move maitri.active-window right
grep -Fqx 'shell moveBarWidget maitri.active-window {"section":"right"}' \
  "$TMPDIR/home/.local/state/maitri/shell-ipc-calls"
pass "bar move accepts a positional target section"

HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" maitri-bar move maitri.active-window left
grep -Fqx 'shell moveBarWidget maitri.active-window {"section":"left"}' \
  "$TMPDIR/home/.local/state/maitri/shell-ipc-calls"
pass "bar move can restore a widget with positional syntax"

if HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" maitri-bar move maitri.active-window left --section right 2>/dev/null; then
  fail "bar move accepted positional and flagged target sections"
fi
pass "bar move rejects conflicting target section syntax"

HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" maitri-bar position bottom
jq -e '
  .bar.position == "bottom" and
  .plugins == []
' "$TMPDIR/home/.config/maitri/shell.json" >/dev/null
pass "shell config sets bar position"

HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" maitri-bar transparent true
jq -e '
...
HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" maitri-bar transparent toggle
jq -e '.bar.transparent == false' "$TMPDIR/home/.config/maitri/shell.json" >/dev/null
pass "shell config toggles bar transparency"

HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" maitri-bar set maitri.bluetooth enabled false --json
grep -Fqx 'shell setBarWidget maitri.bluetooth enabled false {}' \
  "$TMPDIR/home/.local/state/maitri/shell-ipc-calls"
pass "bar set accepts false JSON values"

HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" maitri-bar set maitri.bluetooth optional null --json
grep -Fqx 'shell setBarWidget maitri.bluetooth optional null {}' \
  "$TMPDIR/home/.local/state/maitri/shell-ipc-calls"
pass "bar set accepts null JSON values"

if HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" maitri-bar set maitri.bluetooth broken '{' --json 2>/dev/null; then
  fail "bar set accepted malformed JSON"
fi
pass "bar set rejects malformed JSON"

if HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" maitri-bar set maitri.bluetooth broken 'false null' --json 2>/dev/null; then
...
cat >"$mock_bin/maitri-refresh-config" <<'SH'
#!/bin/bash
set -euo pipefail
...
cp "$MAITRI_PATH/config/$relative_path" "$HOME/.config/$relative_path"
SH

cat >"$mock_bin/maitri-restart-shell" <<'SH'
#!/bin/bash
set -euo pipefail

mkdir -p "$HOME/.local/state/maitri"
touch "$HOME/.local/state/maitri/restart-shell-called"
SH

cat >"$mock_bin/maitri-shell" <<'SH'
#!/bin/bash
[[ ${MAITRI_TEST_SHELL_DOWN:-0} == "1" ]] && exit 1
printf 'ok\n'
SH

cat >"$mock_bin/maitri-installed-service-dropbox" <<'SH'
#!/bin/bash
set -euo pipefail

[[ ${MAITRI_TEST_DROPBOX:-0} == "1" ]]
SH

cat >"$mock_bin/maitri-installed-service-tailscale" <<'SH'
#!/bin/bash
set -euo pipefail

[[ ${MAITRI_TEST_TAILSCALE:-0} == "1" ]]
SH

chmod +x "$mock_bin"/*
mock_path="$mock_bin:$ROOT/bin:$PATH"

HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" PATH="$mock_path" MAITRI_TEST_DROPBOX=0 MAITRI_TEST_TAILSCALE=0 maitri-bar defaults
jq -e --slurpfile defaults "$ROOT/config/maitri/shell.json" '
  .bar == $defaults[0].bar and
  .plugins == []
' "$TMPDIR/home/.config/maitri/shell.json" >/dev/null
pass "bar defaults restores the stock bar"

HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" PATH="$mock_path" MAITRI_TEST_DROPBOX=1 MAITRI_TEST_TAILSCALE=1 maitri-bar defaults
jq -e '
  def ids: map(.id // .);
  (.bar.layout.right | ids) as $right |
  ($right | index("maitri.tray")) as $tray |
  ($right | index("maitri.tailscale") == $tray + 1) and
  ($right | index("maitri.dropbox") == $tray + 2) and
  (.bar.layout.center | ids | index("maitri.tailscale") == null)
' "$TMPDIR/home/.config/maitri/shell.json" >/dev/null
pass "bar defaults places plugins for running optional services"

HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" PATH="$mock_path" \
  MAITRI_TEST_SHELL_DOWN=1 MAITRI_TEST_DROPBOX=1 MAITRI_TEST_TAILSCALE=1 \
  maitri-bar defaults
jq -e '
  def ids: map(.id // .);
  (.bar.layout.right | ids) as $right |
...
HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" PATH="$mock_path" MAITRI_TEST_DROPBOX=0 MAITRI_TEST_TAILSCALE=0 maitri-refresh-shell
jq -e '
  def ids: map(.id // .);
  ([.bar.layout.left, .bar.layout.center, .bar.layout.right] | map(ids) | add) as $all |
  ($all | index("maitri.dropbox") == null) and
  ($all | index("maitri.tailscale") == null)
' "$TMPDIR/home/.config/maitri/shell.json" >/dev/null
pass "shell refresh keeps optional service widgets absent when services are unavailable"

HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" PATH="$mock_path" MAITRI_TEST_DROPBOX=1 MAITRI_TEST_TAILSCALE=1 maitri-refresh-shell
jq -e '
  def ids: map(.id // .);
  (.bar.layout.right | ids) as $right |
  ($right | index("maitri.tray")) as $tray |
...
if grep -RIl 'upgrade-to-quattro\|maitri 4\.0 is upgraded' "$ROOT/migrations" >/dev/null; then
...
cat >"$TMPDIR/home/.config/maitri/shell.json" <<'JSON'
{
  ...
  "bar": {
    "layout": {
      "left": [],
      "center": [
        { "id": "maitri.clock", "formatAlt": "dd MMMM 'W'ww yyyy" },
        { "id": "maitri.weather" }
      ],
      "right": [
...
HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" bash "$clock_migration"
...
before=$(sha256sum "$TMPDIR/home/.config/maitri/shell.json" | awk '{print $1}')
HOME="$TMPDIR/home" MAITRI_PATH="$ROOT" bash "$clock_migration"
after=$(sha256sum "$TMPDIR/home/.config/maitri/shell.json" | awk '{print $1}')
