...
for stub in maitri-launch-webapp maitri-launch-floating-terminal-with-presentation maitri-openclaw-onboard; do
...
"$ROOT/bin/maitri-launch-openclaw"

# Through maitri-openclaw-onboard, never bare `openclaw onboard`, which as of
# 2026.9.1 ends in a browser tab with a foreground gateway and never returns.
grep -q '^maitri-launch-floating-terminal-with-presentation:maitri-openclaw-onboard && \[\[ -f \$HOME/.openclaw/openclaw.json \]\] && maitri-launch-openclaw$' "$TEST_LOG" ||
  fail "OpenClaw launch hands a never-onboarded machine to the wizard" "$(grep floating "$TEST_LOG")"
! grep -q '^maitri-launch-webapp:' "$TEST_LOG" ||
...
grep -q '^maitri-launch-webapp:http://127.0.0.1:18789/#handoff$' "$TEST_LOG" ||
...
grep -q '^maitri-launch-webapp:http://127.0.0.1:18789/#cold-start$' "$TEST_LOG" ||
...
: >"$TEST_LOG"
rc=0
"$ROOT/bin/maitri-launch-openclaw" >/dev/null 2>&1 || rc=$?
...
SCRIPT
chmod +x "$tmp_dir/bin/openclaw"
webapp_calls_before=$(grep -c '^maitri-launch-webapp:' "$TEST_LOG" || true)
rc=0
"$ROOT/bin/maitri-launch-openclaw" >/dev/null 2>&1 || rc=$?
webapp_calls_after=$(grep -c '^maitri-launch-webapp:' "$TEST_LOG" || true)
...
cat >"$tmp_dir/bin/maitri-openclaw-onboard" <<SCRIPT
#!/bin/bash
printf 'maitri-openclaw-onboard:%s\n' "\$*" >>"\$TEST_LOG"
mkdir -p "\$HOME/.openclaw" && touch "\$HOME/.openclaw/openclaw.json"
SCRIPT
chmod +x "$tmp_dir/bin/maitri-openclaw-onboard"
cat >"$tmp_dir/bin/openclaw" <<'SCRIPT'
#!/bin/bash
printf 'openclaw:%s\n' "$*" >>"$TEST_LOG"
...
SCRIPT
chmod +x "$tmp_dir/bin/openclaw"
floating_calls_before=$(grep -c '^maitri-launch-floating-terminal-with-presentation:' "$TEST_LOG" || true)
"$ROOT/bin/maitri-launch-openclaw" --tui

grep -q '^maitri-openclaw-onboard:$' "$TEST_LOG" ||
  fail "--tui onboards in the current terminal" "$(grep 'onboard' "$TEST_LOG")"
floating_calls_after=$(grep -c '^maitri-launch-floating-terminal-with-presentation:' "$TEST_LOG" || true)
...
chmod +x "$tmp_dir/bin/openclaw"
: >"$TEST_LOG"
"$ROOT/bin/maitri-launch-openclaw" --tui --message "Review this project"
