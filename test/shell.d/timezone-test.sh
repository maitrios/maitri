...
timezone_menu="$ROOT/bin/maitri-menu-timezone"
sudoers_file="$ROOT/etc/sudoers.d/maitri-tzupdate"
...
grep -F 'maitri-shell -q maitri.clock refresh' "$timezone_menu" >/dev/null ||
  fail "timezone menu refreshes the namespaced clock IPC target"

! grep -F 'maitri-shell -q Clock refresh' "$timezone_menu" >/dev/null ||
