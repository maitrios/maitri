...
printf 'sudo' >>"$MAITRI_DEV_UNLINK_TEST_LOG"
for arg in "$@"; do
  printf '\t%s' "$arg" >>"$MAITRI_DEV_UNLINK_TEST_LOG"
done
printf '\n' >>"$MAITRI_DEV_UNLINK_TEST_LOG"

if [[ $1 == "tee" ]]; then
  cat >"$MAITRI_DEV_UNLINK_TEST_CONF"
fi
SH
...
printf 'gum' >>"$MAITRI_DEV_UNLINK_TEST_LOG"
for arg in "$@"; do
  printf '\t%s' "$arg" >>"$MAITRI_DEV_UNLINK_TEST_LOG"
done
printf '\n' >>"$MAITRI_DEV_UNLINK_TEST_LOG"
...
run_unlink() {
  HOME="$test_tmp/home" \
    MAITRI_DEV_UNLINK_TEST_LOG="$log_file" \
    MAITRI_DEV_UNLINK_TEST_CONF="$conf_file" \
    PATH="$stub_bin:$PATH" \
    "$ROOT/bin/maitri-dev-unlink" "$@"
}

: >"$log_file"
run_unlink --no-reboot

grep -Fx $'sudo\ttee\t/etc/maitri.conf' "$log_file" >/dev/null ||
  fail "dev unlink writes the package path without rebooting" "$(cat "$log_file")"
[[ $(<"$conf_file") == 'export MAITRI_PATH="/usr/share/maitri"' ]] ||
...
grep -Fx $'sudo\trm\t-f\t/etc/sudoers.d/maitri-dev-path' "$log_file" >/dev/null ||
...
grep -F 'Usage: maitri dev unlink [--no-reboot]' "$test_tmp/invalid.err" >/dev/null ||
