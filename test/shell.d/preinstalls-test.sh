...
marker="$test_home/.local/state/maitri/preinstalls-removed"
pkg_log="$test_tmp/packages"
mkdir -p "$mock_bin" "$test_home/.local/state/maitri"

for command in maitri-webapp-remove-all maitri-tui-remove-all maitri-refresh-applications hyprctl; do
  printf '#!/bin/bash\nexit 0\n' >"$mock_bin/$command"
done

cat >"$mock_bin/gum" <<'SH'
#!/bin/bash
[[ $1 == confirm ]] && exit "${MAITRI_TEST_CONFIRM:-0}"
exit 0
SH
...
#!/bin/bash
printf '%s\n' "$@" >"$MAITRI_TEST_PKG_LOG"
exit "${MAITRI_TEST_PKG_ADD_STATUS:-0}"
SH
...
# $ROOT/bin after the mocks: Remove Preinstalls asks maitri-install-hermes-cli
# whether the wrapper is maitri's rather than matching the marker itself, and
# that is the real command at runtime. The mocks still shadow what they name.
export PATH="$mock_bin:$ROOT/bin:$PATH"
export HOME="$test_home"
export MAITRI_TEST_PKG_LOG="$pkg_log"

# Both scripts restore and remove the same set, and every package in it has to be
# one maitri actually ships, or Remove Preinstalls takes out an app the user
# chose from the menu and Install Preinstalls puts back one we retired.
mapfile -t shipped < <(sed -e 's/[[:space:]]*#.*$//' -e '/^[[:space:]]*$/d' "$ROOT/install/maitri-base.packages")

"$ROOT/bin/maitri-install-preinstalls" >/dev/null
mapfile -t restored <"$pkg_log"

"$ROOT/bin/maitri-remove-preinstalls" >/dev/null
...
for package in "${restored[@]}"; do
  printf '%s\n' "${shipped[@]}" | grep -qxF "$package" ||
    fail "every preinstall is shipped in maitri-base.packages" "$package is not shipped"
done
...
touch "$marker"
MAITRI_TEST_PKG_ADD_STATUS=1 "$ROOT/bin/maitri-install-preinstalls" >/dev/null && status=0 || status=$?
...
rm -f "$marker"
MAITRI_TEST_CONFIRM=1 "$ROOT/bin/maitri-remove-preinstalls" >/dev/null
...
# Hermes' wrapper is only a preinstall when maitri-install-hermes-cli wrote it.
...
printf '%s\n' "#!/bin/bash" "# Written by maitri-install-hermes-cli." >"$hermes"
chmod +x "$hermes"
"$ROOT/bin/maitri-remove-preinstalls" >/dev/null
[[ ! -e $hermes ]] || fail "Remove Preinstalls deletes the maitri Hermes wrapper"
pass "Remove Preinstalls deletes the maitri Hermes wrapper"
