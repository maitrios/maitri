...
snapshot="$ROOT/bin/maitri-snapshot"
...
exit 1
STUB
chmod +x "$fake_bin/maitri-cmd-missing"

cat >"$fake_bin/maitri-version" <<'STUB'
#!/bin/bash
echo 4.0.0
STUB
chmod +x "$fake_bin/maitri-version"
...
grep -qF 'maitri-snapshot create || (($? == 127))' "$ROOT/bin/maitri-update" ||
...
grep -qF 'maitri-snapshot create || (($? == 127))' "$ROOT/bin/maitri-upgrade-to-quattro" ||
  fail "upgrade ignores only the missing-snapper exit code"
grep -qF 'Continuing the upgrade without a snapshot' "$ROOT/bin/maitri-upgrade-to-quattro" ||
