...
grep -Fq $'SUPER + SHIFT + ALT + G	GitHub' <<<"$fresh_output" || fail "default application bindings include preinstalled web apps"
...
if grep -Fq $'SUPER + SHIFT + ALT + G	GitHub' <<<"$removed_output"; then
...
if grep -Fq $'SUPER + SHIFT + ALT + G	GitHub' <<<"$variable_output"; then
