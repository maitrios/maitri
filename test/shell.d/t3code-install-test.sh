...
cat >"$mock_bin/t3" <<'SH'
#!/bin/bash
[[ $1 == "theme" && $2 == "set" && $3 == "maitri" && $4 == "--base-dir" && $5 == "$T3CODE_HOME" ]] || exit 1
palette="$T3CODE_HOME/userdata/themes/maitri.json"
if [[ ! -f $palette ]] || ! jq -e '.name == "maitri" and (.accent | test("^#[0-9a-fA-F]{6}$"))' "$palette" >/dev/null; then
  echo "No published maitri theme"
  exit 1
fi
if [[ ${MAITRI_TEST_T3_FAIL:-0} == "1" ]]; then
  echo "T3 theme selection failed"
  exit 1
fi
echo "T3 selected maitri"
SH
...
run_installer() {
  HOME="$test_home" T3CODE_HOME="$test_tmp/custom t3 home" \
    MAITRI_PATH="$ROOT" PATH="$mock_bin:$ROOT/bin:$PATH" \
    MAITRI_THEME_HEADLESS=1 XDG_RUNTIME_DIR="$test_tmp" \
    "$ROOT/bin/maitri-install-ai-t3-code" >"$test_tmp/output" 2>&1
}

state="$test_home/.local/state/maitri/current"
palette="$test_tmp/custom t3 home/userdata/themes/maitri.json"
settings="$test_tmp/custom t3 home/userdata/settings.json"
mkdir -p "$state/theme"
echo 'spark' >"$state/theme.name"
cp "$ROOT/themes/spark/colors.toml" "$state/theme/colors.toml"
...
grep -q 'T3 selected maitri' "$test_tmp/output" || fail "the published palette is selected"
...
if MAITRI_TEST_T3_FAIL=1 run_installer; then
