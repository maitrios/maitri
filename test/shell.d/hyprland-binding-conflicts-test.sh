...
list_bindings() {
  ...
  HOME="$home" XDG_CONFIG_HOME="$home/.config" XDG_STATE_HOME="$home/.local/state" MAITRI_PATH="$ROOT" MAITRI_BINDING_EPILOGUE="$epilogue" lua <<'LUA'
...
local epilogue = os.getenv("MAITRI_BINDING_EPILOGUE") or ""
if epilogue ~= "" then
  assert(load(epilogue))()
end

-- X11 keycodes are evdev codes plus 8. Only the rows maitri binds by code
...
grep -Fq $'SUPER + SHIFT + ALT + G\tGitHub' <<<"$bindings" || fail "conflict check sees the preinstalled bindings"
