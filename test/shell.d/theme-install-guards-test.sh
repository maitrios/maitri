...
# maitri-theme-install feeds a pasted URL to git and a name derived from it to
# rm, and maitri-theme-remove feeds its argument to rm. Both are exercised here
...
for command in maitri-theme-set maitri-notification-send maitri-menu-select; do
...
install_theme() {
  ...
  HOME="$test_tmp/home" PATH="${2-$mock_bin:$ROOT/bin:$PATH}" \
    MAITRI_TEST_GIT_CALLS="$git_calls" MAITRI_TEST_THEME_CALLS="$theme_calls" \
    bash "$ROOT/bin/maitri-theme-install" "$1" >"$test_tmp/out" 2>&1 || return $?
}

mkdir -p "$test_tmp/home/.config/maitri/themes"
...
for url in "-x" "--upload-pack=touch /tmp/pwned" "ext::sh -c id" "fd::0,1"; do
  if install_theme "$url"; then
    fail "maitri-theme-install refuses the URL '$url'"
  fi

  [[ ! -s $git_calls ]] || fail "maitri-theme-install refuses '$url' before running git" "$(cat "$git_calls")"
done
...
printf '#!/bin/bash\nexit 127\n' >"$missing_checker_bin/maitri-git-url-check"
chmod +x "$missing_checker_bin/maitri-git-url-check"

if install_theme "https://github.com/example/maitri-cool-theme.git" "$missing_checker_bin:$mock_bin:$ROOT/bin:$PATH"; then
  fail "maitri-theme-install refuses a URL it cannot check"
fi

[[ ! -s $git_calls ]] ||
  fail "maitri-theme-install refuses an unchecked URL before running git" "$(cat "$git_calls")"
...
for url in "https://example.com/..git" "https://example.com/.git"; do
  if install_theme "$url"; then
    fail "maitri-theme-install refuses the derived name from '$url'"
  fi
...
for url in \
  "https://example.com/maitri-a';id;'b-theme.git" \
...
# to keep working.
install_theme "https://github.com/example/maitri-tokyo_night.2-theme.git" ||
  fail "maitri-theme-install accepts the punctuation a theme name uses"
grep -Fq "/themes/tokyo_night.2" "$git_calls" ||
  fail "maitri-theme-install derives a name carrying an underscore and a dot" "$(cat "$git_calls")"
...
install_theme "https://github.com/example/maitri-c++-theme.git" ||
  fail "maitri-theme-install accepts a name holding a plus"
grep -Fq "/themes/c++" "$git_calls" ||
  fail "maitri-theme-install derives a name carrying a plus" "$(cat "$git_calls")"

install_theme "https://github.com/example/_private.git" ||
  fail "maitri-theme-install accepts a name starting with an underscore"
grep -Fq "/themes/_private" "$git_calls" ||
  fail "maitri-theme-install derives a name starting with an underscore" "$(cat "$git_calls")"
...
install_theme "git@example.com:maitri-blue-theme.git" ||
  fail "maitri-theme-install accepts a home-relative scp-style URL"
grep -Fq "/themes/blue" "$git_calls" ||
  fail "maitri-theme-install names the theme after the repo, not the whole URL" "$(cat "$git_calls")"

# A colon that is part of a local path, not an scp separator, keeps its prefix.
install_theme "/srv/git:mirrors/maitri-blue-theme.git" ||
  fail "maitri-theme-install accepts a local path holding a colon"
grep -Fq "/themes/blue" "$git_calls" ||
  fail "maitri-theme-install reads a colon after a slash as part of the path" "$(cat "$git_calls")"
...
if locale -a 2>/dev/null | grep -qix 'en_US.utf-\?8'; then
  for locale_name in C en_US.UTF-8; do
    if LC_ALL=$locale_name install_theme "https://github.com/example/maitri-café-theme.git"; then
      fail "maitri-theme-install refuses a non-ASCII theme name under LC_ALL=$locale_name" "$(cat "$git_calls")"
    fi

    [[ ! -s $git_calls ]] ||
      fail "maitri-theme-install refuses a non-ASCII name before running git" "$(cat "$git_calls")"
  done
...
install_theme "host:-s/foo.git" || fail "maitri-theme-install accepts a normal scp-style URL"
grep -Fq -- "-- host:-s/foo.git" "$git_calls" || fail "maitri-theme-install passes the URL after --" "$(cat "$git_calls")"
grep -Fq "/themes/foo" "$git_calls" || fail "maitri-theme-install derives 'foo', not '.git'" "$(cat "$git_calls")"

pass "a dash inside the path does not become a basename option"

# And the ordinary case still works.
install_theme "https://github.com/example/maitri-cool-theme.git" || fail "maitri-theme-install clones a normal URL"
grep -Fq "/themes/cool" "$git_calls" || fail "maitri-theme-install derives the theme name" "$(cat "$git_calls")"
grep -Fxq "cool" "$theme_calls" || fail "maitri-theme-install applies the theme it installed" "$(cat "$theme_calls")"

pass "an ordinary theme URL still clones and applies"

# maitri-theme-remove joins its argument into the path it deletes.
remove_theme() {
  : >"$theme_calls"

  HOME="$test_tmp/home" PATH="$mock_bin:$PATH" MAITRI_TEST_THEME_CALLS="$theme_calls" \
    bash "$ROOT/bin/maitri-theme-remove" "$1" >"$test_tmp/out" 2>&1 || return $?
}

canary="$test_tmp/home/.config/maitri/canary"
printf 'still here\n' >"$canary"

for name in ".." "." "../../evil" ".git"; do
  if remove_theme "$name"; then
    fail "maitri-theme-remove refuses the theme name '$name'"
  fi

  [[ -f $canary ]] || fail "maitri-theme-remove refuses '$name' before removing anything"
done
