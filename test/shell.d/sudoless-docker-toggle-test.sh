...
# (gum confirm), but defer it when MAITRI_DEFER_REBOOT is set (the migration
# reuses them inside `maitri update`, where maitri-update-restart handles it).
...
exit "${GUM_ANSWER:-0}"
STUB
cat >"$stub_bin/maitri-system-reboot" <<'STUB'
#!/bin/bash
touch "${REBOOT_CALLED:?}"
STUB
chmod +x "$stub_bin"/*

reboot_flag="$home/.local/state/maitri/reboot-required"
...
run() { # command STUB_GROUPS GUM_ANSWER DEFER(0|1)
  ...
  local defer_env=()
  [[ ${4:-0} == 1 ]] && defer_env=(MAITRI_DEFER_REBOOT=1)
...
run maitri-remove-security-sudoless-docker "wheel input docker" 0 0
...
run maitri-remove-security-sudoless-docker "wheel input docker" 1 0
...
run maitri-remove-security-sudoless-docker "wheel input docker" 0 1
...
run maitri-remove-security-sudoless-docker "wheel input" 0 0
...
run maitri-setup-security-sudoless-docker "wheel input" 0 0
