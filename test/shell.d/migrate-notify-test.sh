...
cat >"$stub_bin/maitri-migrate" <<'SH'
#!/bin/bash
if [[ ${1:-} == "--pending" && ${MAITRI_TEST_PENDING_MIGRATIONS:-0} == 1 ]]; then
...
SH
chmod +x "$stub_bin/maitri-migrate"
...
cat >"$stub_bin/maitri-notification-wait" <<'SH'
#!/bin/bash
[[ ${MAITRI_TEST_LOCK_DURING_WAIT:-0} == 1 ]] || exit 0

lock="$XDG_RUNTIME_DIR/maitri-update.lock"
held="$lock.held"
rm -f "$held"
: >"$lock"
...
echo "$!" >"$MAITRI_TEST_LOCK_HOLDER_PID"
...
exit 1
SH
chmod +x "$stub_bin/maitri-notification-wait"

cat >"$stub_bin/maitri-notification-send" <<'SH'
#!/bin/bash
[[ ${MAITRI_TEST_NOTIFY_SEND:-send} == "fail" ]] && exit 1
printf '%s\n' "$@" >"$MAITRI_TEST_NOTIFY_ARGS"
SH
...
run_notify() {
  ...
  MAITRI_TEST_PENDING_MIGRATIONS="$1" \
  MAITRI_TEST_NOTIFY_ARGS="$test_tmp/notify-args" \
  MAITRI_TEST_NOTIFY_SEND="${2:-send}" \
  MAITRI_TEST_LOCK_DURING_WAIT="${MAITRI_TEST_LOCK_DURING_WAIT:-0}" \
  MAITRI_TEST_LOCK_HOLDER_PID="$test_tmp/lock-holder-pid" \
    "$ROOT/bin/maitri-migrate-notify"
}
...
grep -q 'maitri has pending migrations' "$test_tmp/pending.err" || fail "migration notifier explains pending migrations without notification system"
...
grep -Fx 'Pending maitri Migrations' "$test_tmp/notify-args" >/dev/null || fail "migration notifier uses pending migrations title"
...
# `maitri update` applies migrations itself, so nothing may notify about them
# while it holds its lock -- a stale trigger firing mid-transaction is exactly
# how the retired maitri-update-user-notify.path used to interrupt updates.
rm -f "$test_tmp/notify-args"
update_lock="$runtime_dir/maitri-update.lock"
: >"$update_lock"
...
pass "migration notifier stays quiet while maitri update holds its lock"

exec {update_lock_fd}>&-

run_notify 1 >/dev/null 2>&1
notify_args_written &&
  grep -Fx 'Pending maitri Migrations' "$test_tmp/notify-args" >/dev/null ||
...
rm -f "$test_tmp/notify-args"
MAITRI_TEST_LOCK_DURING_WAIT=1 run_notify 1 >"$test_tmp/raced.out" 2>"$test_tmp/raced.err"
if [[ -s $test_tmp/lock-holder-pid ]]; then
  kill "$(<"$test_tmp/lock-holder-pid")" 2>/dev/null || true
  for _ in {1..200}; do
    flock -n "$runtime_dir/maitri-update.lock" true 2>/dev/null && break
    sleep 0.05
  done
fi
...
rm -f "$test_tmp/notify-args" /tmp/maitri-update.lock
foreign_lock="$test_tmp/foreign/maitri-update.lock"
...
grep -Fx 'maitri-launch-floating-terminal-with-presentation' "$test_tmp/notify-args" >/dev/null &&
  grep -Fx 'maitri-migrate' "$test_tmp/notify-args" >/dev/null ||
  fail "migration notifier points the click command at maitri-migrate"
