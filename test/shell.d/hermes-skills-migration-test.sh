...
run_migration() {
  HOME="$home" MAITRI_PATH="$ROOT" bash -euo pipefail "$migration" >/dev/null ||
...
rm -rf "$home"
mkdir -p "$home" "$test_dir/empty-maitri"
HOME="$home" MAITRI_PATH="$test_dir/empty-maitri" bash -euo pipefail "$migration" >/dev/null ||
