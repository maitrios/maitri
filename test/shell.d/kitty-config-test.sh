...
run_migration() {
  env HOME="$test_home" MAITRI_PATH="$ROOT" PATH="$ROOT/bin:$PATH" bash -euo pipefail "$migration"
}
...
for command in pkill maitri-restart-shell maitri-hook maitri-notification-send; do
...
run_command() {
  env HOME="$test_home" MAITRI_PATH="$ROOT" PATH="$test_dir/bin:$ROOT/bin:$PATH" "$ROOT/bin/$@"
}

cp "$ROOT/config/kitty/kitty.conf" "$kitty_config"
output=$(run_command maitri-display-text-size)
[[ $output == *"terminal font: 9 pt"* ]] || fail "size report accounts for inherited Kitty default"
run_command maitri-font-set 'Test Font'
run_command maitri-display-text-size 16
grep -qx 'font_family Test Font' "$kitty_config" || fail "font command creates family override"
run_command maitri-font-set Font
...
run_command maitri-display-text-size 18
[[ $(grep -c '^font_size ' "$kitty_config") == "1" ]] || fail "size update avoids duplicate overrides"
run_command maitri-display-text-size reset
...
if "$ROOT/bin/maitri-cmd-present" kitty; then
