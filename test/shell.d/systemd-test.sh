...
sleep_service="$ROOT/default/systemd/user/maitri-sleep-lock.service"
grep -Fx 'ExecStart=/usr/bin/maitri-system-sleep-monitor' "$sleep_service" >/dev/null
...
grep -Fx 'ConditionEnvironment=MAITRI_PATH' "$sleep_service" >/dev/null ||
  fail "sleep lock monitor can start without the maitri shell path"
...
notify_service="$ROOT/default/systemd/user/maitri-migrate-notify.service"
grep -Fx 'ExecStart=/usr/bin/maitri-migrate-notify' "$notify_service" >/dev/null
...
grep -F 'maitri-migrate-notify.service' "$first_run_units" >/dev/null ||
  fail "first-run does not enable the login migration notifier"
grep -F 'maitri-update-user-notify' "$first_run_units" >/dev/null &&
  fail "first-run still enables the retired notifier units"
pass "first-run enables the login-only migration notifier"

fcitx_service="$ROOT/default/systemd/user/maitri-fcitx5.service"
...
oomd_conf="$ROOT/etc/systemd/oomd.conf.d/10-maitri.conf"
