...
export MAITRI_POWERPROFILES_STATE_DIR="$tmp_dir/state"

"$ROOT/bin/maitri-powerprofiles-set" ac balanced
...
"$ROOT/bin/maitri-powerprofiles-set" ac
[[ $(tail -n 1 "$tmp_dir/calls") == "balanced" ]] || fail "power profile restores AC preference"
pass "power profile restores AC preference"

if POWERPROFILES_SET_FAIL=1 "$ROOT/bin/maitri-powerprofiles-set" ac performance; then
...
"$ROOT/bin/maitri-powerprofiles-set" battery performance
...
ON_BATTERY=1 "$ROOT/bin/maitri-powerprofiles-set"
...
rm "$tmp_dir/state/ac"
ON_BATTERY=0 "$ROOT/bin/maitri-powerprofiles-set"
...
"$ROOT/bin/maitri-powerprofiles-set" ac power-saver
"$ROOT/bin/maitri-powerprofiles-init"
...
rg -F '["maitri-powerprofiles-set", pendingPowerSource]' "$ROOT/shell/plugins/services/battery/Service.qml" >/dev/null ||
  fail "battery service applies profiles through maitri command"
pass "battery service applies profiles through maitri command"

rg -F 'maitri-powerprofiles-set autodetect' "$ROOT/shell/plugins/menu/Menu.qml" >/dev/null ||
  fail "power profile menu persists selections through maitri command"
pass "power profile menu persists selections through maitri command"
