...
for command in maitri-shell hyprctl pkill timeout; do
...
PATH="$mock_bin:$PATH" CALL_LOG="$call_log" "$ROOT/bin/maitri-system-lock"
...
[[ ${shutdown[2]} == "pkill -f [o]rg.maitri.screensaver" ]] ||
