...
for (const manifestPath of manifests) {
  ...
  check(String(manifest.id).startsWith('maitri.'), `${manifest.id} must use the first-party namespace`)
  ...
  const clonePaths = manifest.maitri?.clonePaths
  if (clonePaths !== undefined) {
    check(Array.isArray(clonePaths), `${manifest.id} maitri.clonePaths must be an array`)
...
for (const [id, section] of Object.entries({
  'maitri.active-window': 'left',
  'maitri.dropbox': 'right'
})) {
  check(byId[id]?.barWidget?.defaultSection === section, `${id} must default to the ${section} bar section`)
}
check(byId['maitri.media']?.barWidget?.defaultSection === undefined, 'maitri.media must use the center fallback')

for (const id of ['maitri.lock', 'maitri.idle', 'maitri.polkit', 'maitri.notifications', 'maitri.media']) {
