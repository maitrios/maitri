...
const drawn = { moduleName: 'maitri.clock', visible: true, width: 28, height: 81 }
const placeholder = { moduleName: 'maitri.clock', visible: false, width: 0, height: 0 }
...
const internal = { moduleName: 'maitri.audio', visible: true, width: 28, height: 81 }
const external = { moduleName: 'maitri.audio', visible: true, width: 28, height: 81 }
...
assertDeepEqual(bar.entrySettings({ id: 'maitri.clock', format: 'HH:mm' }), { format: 'HH:mm' }, 'bar extracts entry settings')
assertEqual(bar.entryId({ id: 'maitri.clock' }), 'maitri.clock', 'bar extracts object entry ids')
assertEqual(bar.entryId('maitri.clock'), 'maitri.clock', 'bar extracts string entry ids')

const entries = [{ id: 'a' }, { id: 'maitri.tray' }, { id: 'b' }]
assertDeepEqual(bar.pinTrayToInner(entries, 'left').map(bar.entryId), ['a', 'b', 'maitri.tray'], 'bar pins tray to left inner edge')
assertDeepEqual(bar.pinTrayToInner(entries, 'right').map(bar.entryId), ['maitri.tray', 'a', 'b'], 'bar pins tray to right inner edge')
...
const settingsLayout = { left: [{ id: 'maitri.power' }], center: [{ id: 'maitri.clock', format: 'HH:mm' }], right: [] }
assertDeepEqual(
  bar.inlineSettingsDelta(settingsLayout, { left: [{ id: 'maitri.power', showPercentage: true }], center: [{ id: 'maitri.clock', format: 'HH:mm' }], right: [] }),
  [{ region: 'left', index: 0, entry: { id: 'maitri.power', showPercentage: true } }],
...
assertEqual(
  bar.inlineSettingsDelta(settingsLayout, { left: [{ id: 'maitri.clock', format: 'HH:mm' }], center: [{ id: 'maitri.power' }], right: [] }),
  null,
  'bar treats reordered entries as structural'
)
assertEqual(
  bar.inlineSettingsDelta(settingsLayout, { left: [{ id: 'maitri.power' }, { id: 'maitri.battery' }], center: settingsLayout.center, right: [] }),
  null,
...
mkdir -p "$put_tmp/bin"
ln -s "$ROOT/bin/maitri-shell-config" "$put_tmp/bin/maitri-shell-config"

cat >"$put_tmp/bin/maitri-shell" <<'STUB'
#!/bin/bash
case ${MAITRI_TEST_SHELL_STATE:-ready} in
  missing)
    echo "maitri-shell is not running" >&2
    exit 1
    ;;
  starting)
    echo "maitri-shell is not ready" >&2
    exit 1
    ;;
  crashing)
    # Seen coming up, then gone.
    if [[ -e $MAITRI_TEST_SHELL_MARKER ]]; then
      echo "maitri-shell is not running" >&2
    else
      touch "$MAITRI_TEST_SHELL_MARKER"
      echo "maitri-shell is not ready" >&2
    fi
    exit 1
    ;;
  oldshell)
    ...
    if [[ $4 == *"after"* ]]; then
      echo "could not find target widget maitri.clock"
    else
      echo "ok"
    fi
    exit 0
...
echo "ok"
STUB
chmod +x "$put_tmp/bin/maitri-shell"

put_output=$(PATH="$put_tmp/bin:$ROOT/bin:$PATH" MAITRI_TEST_SHELL_STATE=missing \
  MAITRI_SHELL_ABSENT_ATTEMPTS=2 \
  "$ROOT/bin/maitri-bar" put maitri.keyboard-layout --after maitri.clock 2>&1) ||
...
put_output=$(PATH="$put_tmp/bin:$ROOT/bin:$PATH" MAITRI_TEST_SHELL_STATE=spawning \
  MAITRI_TEST_SHELL_MARKER="$put_tmp/spawned" \
  "$ROOT/bin/maitri-bar" put maitri.keyboard-layout --after maitri.clock 2>&1) ||
  fail "put waits for a shell that is being spawned" "$put_output"
[[ $put_output == "maitri.keyboard-layout is on the bar" ]] || fail "put places once the shell answers" "$put_output"
pass "put waits for a shell that is being spawned"

put_output=$(PATH="$put_tmp/bin:$ROOT/bin:$PATH" MAITRI_TEST_SHELL_STATE=starting MAITRI_SHELL_READY_ATTEMPTS=2 \
  "$ROOT/bin/maitri-bar" put maitri.keyboard-layout --after maitri.clock 2>&1) &&
...
put_output=$(PATH="$put_tmp/bin:$ROOT/bin:$PATH" MAITRI_TEST_SHELL_STATE=crashing \
  MAITRI_TEST_SHELL_MARKER="$put_tmp/started" \
  "$ROOT/bin/maitri-bar" put maitri.keyboard-layout --after maitri.clock 2>&1) &&
...
put_output=$(PATH="$put_tmp/bin:$ROOT/bin:$PATH" MAITRI_TEST_SHELL_STATE=oldshell \
  "$ROOT/bin/maitri-bar" put maitri.keyboard-layout --after maitri.clock 2>&1) ||
  fail "put falls back against a shell that has not restarted yet" "$put_output"
[[ $put_output == "maitri.keyboard-layout is on the bar" ]] || fail "put places without the missing neighbour" "$put_output"
pass "put falls back against a shell that has not restarted yet"

put_output=$(PATH="$put_tmp/bin:$ROOT/bin:$PATH" MAITRI_TEST_SHELL_STATE=vanishing \
  MAITRI_TEST_SHELL_MARKER="$put_tmp/vanished" \
  "$ROOT/bin/maitri-bar" put maitri.keyboard-layout --after maitri.clock 2>&1) &&
...
put_output=$(PATH="$put_tmp/bin:$ROOT/bin:$PATH" MAITRI_TEST_SHELL_STATE=unsupported \
  "$ROOT/bin/maitri-bar" put maitri.keyboard-layout --after maitri.clock 2>&1) &&
...
put_output=$(PATH="$put_tmp/bin:$ROOT/bin:$PATH" MAITRI_TEST_SHELL_STATE=scanning \
  MAITRI_TEST_SHELL_MARKER="$put_tmp/scanned" \
  "$ROOT/bin/maitri-bar" put maitri.keyboard-layout --after maitri.clock 2>&1) ||
  fail "put asks again while the shell is still reading its plugins" "$put_output"
[[ $put_output == "maitri.keyboard-layout is on the bar" ]] || fail "put places once the plugins are read" "$put_output"
...
[[ $put_output == "maitri.keyboard-layout is on the bar" ]] || fail "put reports the placed widget" "$put_output"
