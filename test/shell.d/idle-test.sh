...
HOME="$test_home" "$ROOT/bin/maitri-toggle-idle" stay-awake >/dev/null
[[ -f $test_home/.local/state/maitri/indicators/stay-awake ]] || fail "Stay Awake toggle persists enabled state"

HOME="$test_home" "$ROOT/bin/maitri-toggle-idle" allow-idle >/dev/null
[[ ! -f $test_home/.local/state/maitri/indicators/stay-awake ]] || fail "Stay Awake toggle persists disabled state"

if rg -q 'maitri-shell' "$ROOT/bin/maitri-toggle-idle"; then
