...
# Every plugin under ~/.config/maitri/plugins hot-reloads, whoever wrote it.
hot_reload_id="acme.hot-reload"
hot_reload_dir="$test_home/.config/maitri/plugins/$hot_reload_id"
...
cat >"$hot_reload_dir/manifest.json" <<JSON
{
  ...
  "maitri": {"clonedFrom": "maitri.emojis"}
}
JSON
...
keep_service_dir="$test_home/.config/maitri/plugins/$keep_service_id"
...
victim_service_dir="$test_home/.config/maitri/plugins/$victim_service_id"
...
media_clone_dir="$test_home/.config/maitri/plugins/$media_clone_id"
...
cat >"$media_clone_dir/manifest.json" <<JSON
{
  ...
  "maitri": {"clonedFrom": "maitri.media"}
}
JSON
...
Item {
  ...
  IpcHandler {
    ...
    function summonOsd(): string {
      return root.shell && root.shell.summon("maitri.osd", "{}") ? "true" : "false"
    }
  }
}
QML
  ...
  IpcHandler {
    ...
    function probeOwnService(): string {
      var service = root.bar && root.bar.shell
        ? root.bar.shell.firstPartyServiceFor("maitri.media") : null
...
review_bar_dir="$test_home/.config/maitri/plugins/$review_bar_id"
  ...
  IpcHandler {
    ...
    function probeMediaProxy(): string {
      var service = root.shell
        ? root.shell.firstPartyServiceFor("maitri.media") : null
    ...
    function probeMediaWidgetSummon(): string {
      ...
      return JSON.stringify({
        entryFacade: !!entryFacade,
        osdSummoned: entryFacade ? entryFacade.summon("maitri.osd", "{}") : false,
        foreignSummoned: entryFacade ? entryFacade.summon("maitri.lock", "{}") : false
      })
    }
...
jq -e '
  map(.id) as $ids |
  all(["maitri.menu", "maitri.notifications", "maitri.clock", "maitri.osd"][]; $ids | index(.)) and
...
[[ $(shell_ipc shell summon maitri.emojis "{}") == "ok" ]] ||
  fail_with_log "calls to a cloned source id do not reach its enabled clone"
shell_ipc_quiet shell hide maitri.emojis >/dev/null
...
for _ in {1..80}; do
  ...
  geometry=$(shell_ipc shell debugBarGeometry 2>/dev/null || true)
  if jq -e 'all(.bar.layout.right[]; (.id // .) != "maitri.audio")' <<<"$shell_config" >/dev/null 2>&1 && \
     jq -e 'all(.[]; .id != "maitri.audio")' <<<"$geometry" >/dev/null 2>&1; then
    break
  fi
  if ! kill -0 "$QS_PID" 2>/dev/null; then
...
jq -e 'all(.bar.layout.right[]; (.id // .) != "maitri.audio")' <<<"$shell_config" >/dev/null || {
  printf 'Shell config after reload:\n%s\n' "$shell_config" | jq . >&2
  fail_with_log "plugin disable reloads shell config"
}

jq -e 'all(.[]; .id != "maitri.audio")' <<<"$geometry" >/dev/null || {
...
bar_put() {
  HOME="$test_home" MAITRI_PATH="$test_root" PATH="$ROOT/bin:$PATH" "$ROOT/bin/maitri-bar" put "$@"
}
...
bar_put maitri.keyboard-layout --after maitri.clock >/dev/null
for _ in {1..80}; do
  [[ $(center_ids) == *maitri.keyboard-layout* ]] && break
...
jq -e '
  ...
  | ($ids | index("maitri.clock")) as $clock
  | ($ids | index("maitri.keyboard-layout")) as $widget
...
placed=$(center_ids)
bar_put maitri.keyboard-layout --section right >/dev/null
sleep 0.5
[[ $(center_ids) == "$placed" ]] ||
  fail_with_log "bar put left a widget already on the bar alone (was $placed, now $(center_ids))"
jq -e 'all(.bar.layout.right[]; (.id // .) != "maitri.keyboard-layout")' \
  <<<"$(shell_ipc shell listShellConfig)" >/dev/null ||
...
[[ $(shell_ipc shell setBarWidget maitri.clock format '"HH:mm:ss"' '{}') == "ok" ]] ||
...
for _ in {1..80}; do
  ...
  if jq -e 'any(.layout.center[]; (.id // .) == "maitri.clock" and .format == "HH:mm:ss")' \
    <<<"$updated_snapshot" >/dev/null 2>&1; then
    break
  fi
  sleep 0.1
done
jq -e 'any(.layout.center[]; (.id // .) == "maitri.clock" and .format == "HH:mm:ss")' \
