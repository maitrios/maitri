...
grep -F 'maitri-snapshot create || (($? == 127))' "$upgrade_to_quattro" >/dev/null
pass "maitri 4 upgrade snapshots the system before mutation"
...
grep -F 'pacman -Syy --noconfirm archlinux-keyring maitri-keyring' "$upgrade_to_quattro" >/dev/null
if grep -F 'pacman -Sy --noconfirm archlinux-keyring maitri-keyring' "$upgrade_to_quattro" >/dev/null; then
  fail "maitri 4 upgrade forces a database refresh before installing keyrings"
fi
pass "maitri 4 upgrade forces a database refresh before installing keyrings"

grep -F 'pacman -Syu --needed' "$upgrade_to_quattro" >/dev/null
grep -F 'maitri-update-aur-pkgs' "$upgrade_to_quattro" >/dev/null
grep -F 'maitri-update-available' "$upgrade_to_quattro" >/dev/null
grep -F 'maitri-update-mise' "$upgrade_to_quattro" >/dev/null
grep -F 'run_final_system_package_upgrade' "$upgrade_to_quattro" >/dev/null
pass "maitri 4 upgrade completes package update checks"

grep -F 'run_post_upgrade_migrations' "$upgrade_to_quattro" >/dev/null
grep -F 'maitri-migrate' "$upgrade_to_quattro" >/dev/null
...
(( final_upgrade_line < migrations_line )) ||
  fail "maitri migrations run after the final package upgrade"
pass "maitri 4 upgrade applies packaged migrations"

if grep -F 'skip-first-run-update-notification' "$upgrade_to_quattro" >/dev/null; then
  fail "maitri 4 upgrade does not use notification-specific first-run state"
fi
pass "maitri 4 upgrade completes first-run as one lifecycle"
...
pass "maitri 4 upgrade completes first-run and migrates legacy completion markers"
...
if grep -F '"$root/bin/maitri-done"' "$upgrade_to_quattro" >/dev/null; then
  fail "maitri 4 upgrade writes completion markers without the packaged maitri-done"
fi
pass "maitri 4 upgrade writes completion markers without the packaged maitri-done"

for guarded_step in maitri-refresh-applications 'maitri-bar defaults'; do
  grep -F "run_as_user_maitri $guarded_step ||" "$upgrade_to_quattro" >/dev/null ||
    fail "maitri 4 upgrade survives a packaged tree without $guarded_step"
done
pass "maitri 4 upgrade survives a packaged tree missing top-level commands"

grep -F 'configure_snapper_policy' "$upgrade_to_quattro" >/dev/null
grep -F '/usr/share/maitri/install/config/snapper.sh' "$upgrade_to_quattro" >/dev/null
grep -F 'bash -euo pipefail "$snapper_config_script"' "$upgrade_to_quattro" >/dev/null
pass "maitri 4 upgrade normalizes Snapper retention"

grep -F 'configure_lock_authentication' "$upgrade_to_quattro" >/dev/null
grep -F 'MAITRI_INSTALL_USER="$target_user"' "$upgrade_to_quattro" >/dev/null
grep -F '"$apply_lock"' "$upgrade_to_quattro" >/dev/null
pass "maitri 4 upgrade configures lock screen authentication for the target user"

root_path_count=$(awk '/^root_path=/{ count++ } END { print count + 0 }' "$upgrade_to_quattro")
(( root_path_count == 1 )) || fail "maitri 4 upgrade defines exactly one root command path"
grep -Fx 'root_path=/usr/share/maitri/bin:/usr/local/bin:/usr/bin:/bin' "$upgrade_to_quattro" >/dev/null ||
  fail "maitri 4 upgrade limits root command lookup to trusted system directories"
if grep -E '^root_path=.*(target_home|\.local/bin)' "$upgrade_to_quattro" >/dev/null; then
  fail "maitri 4 upgrade does not put the target user's bin directory on the root command path"
fi
grep -Fx 'package_path="$root_path:$target_home/.local/bin"' "$upgrade_to_quattro" >/dev/null ||
  fail "maitri 4 upgrade retains the target user's bin directory for user commands"
...
(( lock_path_assignment_count == 1 )) ||
  fail "maitri 4 upgrade gives the privileged lock helper exactly one command path"
grep -Fx '    PATH="$root_path" \' <<<"$lock_authentication_body" >/dev/null ||
  fail "maitri 4 upgrade gives the privileged lock helper the trusted root path"
if grep -E '(package_path|target_home|\.local/bin)' <<<"$lock_authentication_body" >/dev/null; then
  fail "maitri 4 upgrade does not give the privileged lock helper the target user's path"
fi
...
(( firewall_path_assignment_count == 1 )) ||
  fail "maitri 4 upgrade gives the privileged firewall helper exactly one command path"
grep -Fx '  as_root env MAITRI_PATH=/usr/share/maitri PATH="$root_path" \' <<<"$firewall_body" >/dev/null ||
  fail "maitri 4 upgrade gives the privileged firewall helper the trusted root path"
if grep -E '(package_path|target_home|\.local/bin)' <<<"$firewall_body" >/dev/null; then
  fail "maitri 4 upgrade does not give the privileged firewall helper the target user's path"
fi

user_maitri_body=$(function_body run_as_user_maitri)
grep -F 'PATH="$package_path"' <<<"$user_maitri_body" >/dev/null ||
  fail "maitri 4 upgrade retains the package and user path for target-user commands"
if grep -F 'PATH="$root_path"' <<<"$user_maitri_body" >/dev/null; then
  fail "maitri 4 upgrade does not narrow target-user commands to the root-only path"
fi
pass "maitri 4 upgrade separates privileged and target-user command paths"

grep -F 'install/helpers/browser-policy.sh' "$upgrade_to_quattro" >/dev/null ||
  fail "maitri 4 upgrade uses the shared browser-policy helper"
grep -F 'as_root test -f "$browser_policy_helper"' "$upgrade_to_quattro" >/dev/null ||
  fail "maitri 4 upgrade survives a packaged tree without the browser-policy helper"
if grep -F 'browser_policy_setup_group' "$upgrade_to_quattro" >/dev/null; then
  fail "maitri 4 upgrade does not create a browser-policy group"
fi
grep -F 'browser_policy_setup_dir /etc/chromium/policies/managed' "$upgrade_to_quattro" >/dev/null ||
  fail "maitri 4 upgrade creates a root-owned Chromium policy directory"
grep -F 'BROWSER_POLICY_MANAGED_DIRS' "$upgrade_to_quattro" >/dev/null ||
  fail "maitri 4 upgrade hardens every Chromium-family policy directory"
grep -F 'run_as_user_maitri maitri-theme-set-browser' "$upgrade_to_quattro" >/dev/null ||
  fail "maitri 4 upgrade rewrites browser theme colour after a headless theme-set"
if grep -E 'install -d -m 0?[27]?777 /etc/.*/policies|chmod a\+rw|2775' "$upgrade_to_quattro" >/dev/null; then
  fail "maitri 4 upgrade does not create a world-writable Chromium policy directory"
fi
pass "maitri 4 upgrade locks the Chromium policy directory to root"

grep -F 'MAITRI_UPGRADE_TO_QUATTRO_LIVE=1' "$upgrade_to_quattro" >/dev/null
...
pass "maitri 4 upgrade retires systemd-networkd for NetworkManager"
...
grep -F 'fail "maitri migrations did not complete.' <<<"$migrations_body" >/dev/null ||
  fail "maitri 4 upgrade fails when a migration cannot complete"
grep -F 'maitri-migrate --pending' <<<"$migrations_body" >/dev/null ||
  fail "maitri 4 upgrade verifies that migrations actually completed"
grep -F 'fail "maitri migrations are still pending.' <<<"$migrations_body" >/dev/null ||
  fail "maitri 4 upgrade fails when a successful migration command leaves pending work"
grep -F 'pending_status != 1' <<<"$migrations_body" >/dev/null ||
  fail "maitri 4 upgrade distinguishes no pending work from a failed verification"
grep -F 'fail "Could not verify that maitri migrations completed.' <<<"$migrations_body" >/dev/null ||
  fail "maitri 4 upgrade fails when it cannot verify migration state"
if grep -F 'return 0' <<<"$migrations_body" >/dev/null || grep -F 'warn ' <<<"$migrations_body" >/dev/null; then
  fail "maitri 4 upgrade does not continue past failed migrations"
fi
...
exercise_post_upgrade_migrations 0 1 >/dev/null 2>&1 ||
  fail "maitri 4 upgrade accepts a completed migration queue"
if exercise_post_upgrade_migrations 1 1 >/dev/null 2>&1; then
  fail "maitri 4 upgrade accepts a failed migration"
fi
if exercise_post_upgrade_migrations 0 0 >/dev/null 2>&1; then
  fail "maitri 4 upgrade accepts pending migrations"
fi
if exercise_post_upgrade_migrations 0 2 >/dev/null 2>&1; then
  fail "maitri 4 upgrade accepts a failed pending-state check"
fi
pass "maitri 4 upgrade cannot finish with pending migrations"

if function_body cleanup_retired_services | grep -F 'systemctl disable iwd' >/dev/null; then
  fail "maitri 4 upgrade does not retire iwd in a step separate from the NetworkManager enable"
fi
grep -A1 -F '  enable_system_service NetworkManager.service' "$upgrade_to_quattro" |
  ...
  fail "maitri 4 upgrade retires iwd in the step that enables NetworkManager"
pass "maitri 4 upgrade switches from iwd to NetworkManager atomically"
...
grep -Fx 'trap cleanup_on_exit EXIT' "$upgrade_to_quattro" >/dev/null ||
  fail "maitri 4 upgrade reports an aborted run instead of exiting silently"
...
grep -F 'exit "$exit_status"' <<<"$cleanup_body" >/dev/null ||
  fail "maitri 4 upgrade preserves the failing exit status"
grep -F '>&2' <<<"$cleanup_body" >/dev/null ||
  fail "maitri 4 upgrade reports an aborted run on stderr"
...
pass "maitri 4 upgrade reports an aborted run instead of exiting silently"
...
! grep -q 'start_maitri_shell_session' "$upgrade_to_quattro" ||
  fail "maitri 4 upgrade does not start the shell in the session it is replacing"
! grep -q 'stop_retired_session_processes' "$upgrade_to_quattro" ||
  fail "maitri 4 upgrade leaves the retired session processes running until reboot"
pass "maitri 4 upgrade leaves the maitri 3 session alone until the reboot"

grep -F 'maitri-bar defaults' "$upgrade_to_quattro" >/dev/null
pass "maitri 4 upgrade restores service-aware bar defaults"
...
pass "maitri 4 upgrade backfills hardware support from the legacy release"

grep -F 'maitri-refresh-applications' "$upgrade_to_quattro" >/dev/null
pass "maitri 4 upgrade refreshes application launchers"

grep -F '/etc/systemd/system.conf.d/99-maitri-nofile.conf' "$upgrade_to_quattro" >/dev/null
grep -F '/etc/systemd/user.conf.d/99-maitri-nofile.conf' "$upgrade_to_quattro" >/dev/null
pass "maitri 4 upgrade removes stale nofile drop-ins"

cmdline_line=$(grep -n '^preserve_kernel_cmdline_root$' "$upgrade_to_quattro" | cut -d: -f1)
packages_line=$(grep -n '^install_maitri_quattro_packages$' "$upgrade_to_quattro" | cut -d: -f1)
...
pass "maitri 4 upgrade preserves the kernel cmdline root parameters"
...
pass "maitri 4 upgrade asks limine-entry-tool whether root= survives"
...
pass "maitri 4 upgrade repair path refuses a partial dm-crypt cmdline"
...
grep -F "as_root find /boot/EFI/Linux -maxdepth 1 -name 'maitri_linux*.efi'" "$upgrade_to_quattro" >/dev/null
...
pass "maitri 4 upgrade verifies the UKIs and refuses to reboot unverified"
