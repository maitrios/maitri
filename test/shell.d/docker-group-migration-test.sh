...
#
# The real maitri-remove-security-sudoless-docker and maitri-state run here
...
mkdir -p "$home/.local/share/applications" "$maitri_path/applications" "$stub_bin"

printf 'NEW-LAUNCHER\n' >"$maitri_path/applications/Docker.desktop"
...
# must still NOT reboot (it defers to maitri-update-restart), so neither should
# be reached.
...
touch "${REBOOT_CALLED:?}"
STUB
chmod +x "$stub_bin/id" "$stub_bin/sudo" "$stub_bin/gpasswd" "$stub_bin/gum" "$stub_bin/maitri-system-reboot"
...
run_migration() {
  ...
  HOME="$home" MAITRI_PATH="$maitri_path" USER="tester" STUB_GROUPS="$1" \
