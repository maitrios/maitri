...
cat >"$TMPDIR/bin/maitri-notification-send" <<'SH'
#!/bin/bash
exit 0
SH

chmod +x "$TMPDIR/bin/systemctl" "$TMPDIR/bin/maitri-notification-send"

test_home="$TMPDIR/home"
flag="$test_home/.local/state/maitri/toggles/crash-capture-off"

toggle_crash_capture() {
  PATH="$TMPDIR/bin:$ROOT/bin:$PATH" \
  SYSTEMCTL_LOG="$SYSTEMCTL_LOG" \
  HOME="$test_home" \
    "$ROOT/bin/maitri-toggle-crash-capture"
}

toggle_crash_capture
[[ -f $flag ]] || fail "crash capture toggle disables the watcher"
grep -Fqx -- "--user stop maitri-crash-watch.service" "$SYSTEMCTL_LOG" ||
...
grep -Fqx -- "--user start maitri-crash-watch.service" "$SYSTEMCTL_LOG" ||
...
service="$ROOT/default/systemd/user/maitri-crash-watch.service"
grep -Fx 'ConditionPathExists=!%h/.local/state/maitri/toggles/crash-capture-off' "$service" >/dev/null ||
...
grep -F 'maitri-crash-watch.service' "$ROOT/install/user/first-run/enable-user-units.sh" >/dev/null ||
...
const items = menu.parseMenuJsonc(fs.readFileSync(path.join(root, 'default/maitri/maitri-menu.jsonc'), 'utf8'))
const byId = Object.fromEntries(items.map(item => [item.id, item]))

assertEqual(
  byId['trigger.toggle.crash-capture'].action,
  'maitri-toggle-crash-capture',
