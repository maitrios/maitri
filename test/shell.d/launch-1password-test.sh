...
#!/bin/bash
[[ ${MAITRI_TEST_INSTALLED:-false} == "true" ]]
SH
...
PATH="$mock_bin:$PATH" MAITRI_TEST_INSTALLED=true MAITRI_TEST_LOG="$launch_log" \
  bash "$ROOT/bin/maitri-launch-1password"
...
PATH="$mock_bin:$PATH" MAITRI_TEST_INSTALLED=false MAITRI_TEST_LOG="$launch_log" \
  bash "$ROOT/bin/maitri-launch-1password"
grep -Fxq 'install:maitri-install-service-1password' "$launch_log" ||
  fail "1Password launcher starts the installer when missing"
pass "1Password launcher starts the installer when missing"

grep -Fq '{ maitri = "1password" }' "$ROOT/default/hypr/bindings/applications.lua" ||
