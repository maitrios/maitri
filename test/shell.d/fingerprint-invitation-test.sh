...
if [[ -f /etc/pam.d/maitri-lock-fingerprint ]]; then
...
log_file=$(mktemp)
hw_marker=$(mktemp -u)
hook_path="$test_home/.config/maitri/hooks/post-update.d/setup-fingerprint.hook"
...
cat >"$test_bin/maitri-hw-fingerprint" <<'EOF'
#!/bin/bash
[[ -f $TEST_HW_MARKER ]]
EOF
chmod +x "$test_bin/maitri-hw-fingerprint"
...
[[ ! -f $test_home/.local/state/maitri/done/fingerprint-setup-invitation ]] || fail "fingerprint invitation stays pending without a reader"
[[ ! -s $log_file ]] || fail "fingerprint invitation does nothing without a reader"

touch "$hw_marker"
run_invitation_hook

[[ -f $test_home/.local/state/maitri/done/fingerprint-setup-invitation ]] || fail "fingerprint invitation records completion"
...
grep -qx 'exec:maitri-launch-floating-terminal-with-presentation maitri-setup-security-fingerprint' "$log_file" ||
