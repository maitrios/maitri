...
# maitri-theme-set-hermes writes a file another program parses and asks that
...
#!/bin/bash
echo "check" >>"$MAITRI_TEST_HERMES_CALLS"
[[ $1 == "--check" && ${MAITRI_TEST_HERMES_READY:-0} == "1" ]]
SH

# A theme switch finishes the hand-over only for the desktop app maitri
# installed; --activate is asked for by name and does not look.
cat >"$mock_bin/maitri-pkg-present" <<'SH'
#!/bin/bash
[[ $1 == "hermes-desktop" && ${MAITRI_TEST_DESKTOP_INSTALLED:-1} == "1" ]]
SH
...
#!/bin/bash
echo "PATH hermes ran: $*" >>"$MAITRI_TEST_HERMES_CALLS"
exit 1
SH

cat >"$mock_bin/hermes-stub" <<'SH'
#!/bin/bash
printf '%s\n' "$*" >>"$MAITRI_TEST_HERMES_CALLS"
if [[ $1 == "config" && $2 == "get" ]]; then
  [[ ${MAITRI_TEST_HERMES_GET_FAILS:-0} == 0 ]] || exit 1
  printf '%s\n' "${MAITRI_TEST_HERMES_SKIN-default}"
fi
if [[ $1 == "config" && $2 == "set" ]]; then
  [[ ${MAITRI_TEST_HERMES_SET_FAILS:-0} == 0 ]] || exit 1
fi
SH

# A refresh re-stages the current theme, which is where the skin gets rendered.
cat >"$mock_bin/maitri-theme-refresh" <<'SH'
#!/bin/bash
echo "refresh" >>"$MAITRI_TEST_HERMES_CALLS"
printf 'name: maitri\ndescription: maitri system theme\ncolors:\n  background: "#1a1b26"\n' \
  >"$HOME/.local/state/maitri/current/theme/hermes.yaml"
SH
...
#!/bin/bash
printf 'sleep %s\n' "$1" >>"$MAITRI_TEST_HERMES_CALLS"
if [[ $1 == 60 && -n ${MAITRI_TEST_SWAP_SOURCE:-} ]]; then
  printf '%s\n' "$MAITRI_TEST_SWAP_SOURCE" >"$HOME/.local/state/maitri/current/theme/hermes.yaml"
fi
if [[ $1 == 60 && ${MAITRI_TEST_DROP_SKINS:-0} == 1 ]]; then
  rm -f "$HOME/.hermes/skins/maitri.yaml" "$HOME"/.hermes/profiles/*/skins/maitri.yaml
fi
SH

chmod +x "$mock_bin"/*

good_skin='name: maitri
description: maitri system theme
colors:
  background: "#1a1b26"
  ui_text: "#a9b1d6"
...
skin="$hermes_home/skins/maitri.yaml"
...
reset_home() {
  ...
  rm -rf "$test_home"
  mkdir -p "$test_home/.local/state/maitri/current/theme" "$test_home/.local/bin" "$hermes_home/skills"
  ...
  printf '%s\n' "$source" >"$test_home/.local/state/maitri/current/theme/hermes.yaml"
}

run_hook() {
  MAITRI_TEST_HERMES_READY="${MAITRI_TEST_HERMES_READY:-0}" \
    MAITRI_TEST_HERMES_SKIN="${MAITRI_TEST_HERMES_SKIN-default}" \
    MAITRI_TEST_HERMES_GET_FAILS="${MAITRI_TEST_HERMES_GET_FAILS:-0}" \
    MAITRI_TEST_HERMES_SET_FAILS="${MAITRI_TEST_HERMES_SET_FAILS:-0}" \
    MAITRI_TEST_HERMES_CALLS="$hermes_calls" \
    MAITRI_TEST_DESKTOP_INSTALLED="${MAITRI_TEST_DESKTOP_INSTALLED:-1}" \
    MAITRI_TEST_SWAP_SOURCE="${MAITRI_TEST_SWAP_SOURCE:-}" \
    MAITRI_TEST_DROP_SKINS="${MAITRI_TEST_DROP_SKINS:-0}" \
    PATH="$mock_bin:$PATH" \
    HOME="$test_home" \
    HERMES_HOME='' \
    "$ROOT/bin/maitri-theme-set-hermes" "$@"
}
...
reset_home
run_hook
[[ ! -e $hermes_home/skins ]] || fail "a Hermes home that only holds the maitri skill gets no skin"
...
diff -q "$test_home/.local/state/maitri/current/theme/hermes.yaml" "$skin" >/dev/null ||
  fail "the generated skin is published to ~/.hermes/skins/maitri.yaml"
diff -q "$skin" "$hermes_home/profiles/work/skins/maitri.yaml" >/dev/null ||
  fail "an existing Hermes profile gets the skin too"
[[ $(ls "$hermes_home/skins") == "maitri.yaml" ]] || fail "no temporary file is left beside the skin"
...
[[ -z $(ls -A "$skin") && $(ls "$hermes_home/skins") == "maitri.yaml" ]] ||
...
reset_home --set-up
MAITRI_TEST_HERMES_READY=1 run_hook 2>"$test_tmp/stderr"
[[ $(cat "$hermes_calls") == $'check\nconfig get display.skin\nconfig set display.skin maitri' ]] ||
...
reset_home --on maitri
MAITRI_TEST_HERMES_READY=1 run_hook
...
reset_home --set-up
MAITRI_TEST_HERMES_READY=1 MAITRI_TEST_DESKTOP_INSTALLED=0 run_hook
[[ -f $skin ]] || fail "a Hermes installed some other way still gets the skin published"
[[ ! -s $hermes_calls ]] || fail "a theme switch does not touch a Hermes maitri did not install as the app" "$(cat "$hermes_calls")"
pass "a theme switch activates only for the desktop app maitri installed"

reset_home --set-up
MAITRI_TEST_HERMES_READY=1 MAITRI_TEST_DESKTOP_INSTALLED=0 run_hook --activate 2>/dev/null
grep -Fxq 'config set display.skin maitri' "$hermes_calls" ||
  fail "--activate switches whichever Hermes it is asked about" "$(cat "$hermes_calls")"
pass "--activate does not ask which Hermes it is"

reset_home --on ares
MAITRI_TEST_HERMES_READY=1 MAITRI_TEST_HERMES_SKIN=ares run_hook 2>"$test_tmp/stderr"
[[ -f $skin ]] || fail "a chosen skin still gets the maitri skin published beside it"
...
MAITRI_TEST_HERMES_READY=1 run_hook 2>"$test_tmp/stderr" || fail "a profile that cannot take the skin does not fail the hook"
...
grep -q 'hermes config set display.skin maitri' "$test_tmp/stderr" || fail "an unready Hermes gets the command that finishes the job"
pass "--activate publishes but does not run a Hermes that is not ready"

reset_home --set-up
MAITRI_TEST_HERMES_READY=1 run_hook --activate 2>"$test_tmp/stderr"
[[ $(cat "$hermes_calls") == $'check\nconfig get display.skin\nconfig set display.skin maitri' ]] ||
  fail "a ready Hermes is asked for its skin and then to switch" "$(cat "$hermes_calls")"
grep -q 'on the maitri skin' "$test_tmp/stderr" || fail "--activate reports success"
pass "--activate goes through hermes config set when Hermes runs"

reset_home --on maitri
MAITRI_TEST_HERMES_READY=1 MAITRI_TEST_HERMES_SKIN=maitri run_hook --activate 2>/dev/null
grep -Fxq 'config set display.skin maitri' "$hermes_calls" ||
...
for chosen in maitri ""; do
  reset_home --set-up
  MAITRI_TEST_HERMES_READY=1 MAITRI_TEST_HERMES_SKIN="$chosen" run_hook --activate 2>/dev/null
  grep -Fxq 'config set display.skin maitri' "$hermes_calls" ||
    fail "the default and the maitri skin are both replaced" "skin='$chosen': $(cat "$hermes_calls")"
done
pass "--activate replaces Hermes' default skin"

reset_home --set-up
MAITRI_TEST_HERMES_READY=1 MAITRI_TEST_HERMES_SKIN=ares run_hook --activate 2>"$test_tmp/stderr"
[[ -f $skin ]] || fail "a chosen skin still gets the maitri skin published beside it"
...
reset_home --set-up
MAITRI_TEST_HERMES_READY=1 MAITRI_TEST_HERMES_GET_FAILS=1 run_hook --activate 2>"$test_tmp/stderr" || fail "a Hermes that does not answer is not an error"
...
MAITRI_TEST_HERMES_READY=1 MAITRI_TEST_HERMES_SET_FAILS=1 run_hook --wait 2>"$test_tmp/stderr" || fail "a Hermes that refuses the write is not an error"
...
reset_home --set-up
rm "$test_home/.local/state/maitri/current/theme/hermes.yaml"
run_hook
...
grep -q 'Select an maitri theme' "$test_tmp/stderr" || fail "a missing theme is reported"
pass "--activate fails without a current theme to render the skin from"

reset_home --set-up
rm "$test_home/.local/state/maitri/current/theme/hermes.yaml"
echo tokyo-night >"$test_home/.local/state/maitri/current/theme.name"
MAITRI_TEST_HERMES_READY=1 run_hook --activate 2>/dev/null
...
grep -Fxq 'config set display.skin maitri' "$hermes_calls" || fail "the freshly rendered skin is activated" "$(cat "$hermes_calls")"
...
mkdir -p "$hermes_home/hermes-agent"
touch "$hermes_home/hermes-agent/.hermes-bootstrap-complete"
MAITRI_TEST_HERMES_READY=1 run_hook --wait 2>/dev/null
[[ $(cat "$hermes_calls") == $'check\nconfig get display.skin\nconfig set display.skin maitri\nsleep 60' ]] ||
...
touch "$hermes_home/hermes-agent/.hermes-bootstrap-complete"
MAITRI_TEST_HERMES_READY=1 MAITRI_TEST_SWAP_SOURCE=$'name: maitri\ncolors:\n  background: "#1a1b26"\nbanner_logo: "evil"' run_hook --wait 2>/dev/null
...
mkdir -p "$hermes_home/hermes-agent" "$hermes_home/profiles/work"
touch "$hermes_home/hermes-agent/.hermes-bootstrap-complete"
MAITRI_TEST_HERMES_READY=1 MAITRI_TEST_DROP_SKINS=1 run_hook --wait 2>/dev/null
[[ -f $skin && -f $hermes_home/profiles/work/skins/maitri.yaml ]] ||
...
reset_home
MAITRI_TEST_HERMES_READY=1 run_hook --wait 2>"$test_tmp/stderr"
