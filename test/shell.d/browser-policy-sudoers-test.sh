...
helper="$ROOT/bin/maitri-theme-set-browser-policy"
setter="$ROOT/bin/maitri-theme-set-browser"
sudoers_file="$ROOT/etc/sudoers.d/maitri-theme-browser"
rule='%wheel ALL=(root) NOPASSWD: /usr/bin/maitri-theme-set-browser-policy [0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]'
...
grep -Fx 'PACKAGED_PATH=/usr/bin/maitri-theme-set-browser-policy' "$helper" >/dev/null ||
  fail "maitri-theme-set-browser-policy elevates the path the sudoers rule names"

grep -E 'sudo -n -l -l' "$helper" >/dev/null ||
  fail "maitri-theme-set-browser-policy reads the grant from the long sudo listing"

grep -Eq '^\s*export PATH=/usr/local/sbin:/usr/local/bin:/usr/bin' "$helper" ||
  fail "maitri-theme-set-browser-policy pins PATH to trusted system directories when it holds root"
gated=$(grep -A1 -E '^if \(\( EUID == 0 \)\); then$' "$helper" || true)
[[ $gated == *"export PATH=/usr/local/sbin:/usr/local/bin:/usr/bin"* ]] ||
  fail "maitri-theme-set-browser-policy gates the trusted-PATH pin on holding root"
...
grep -F 'install -m 0644 -o root -g root -T' "$helper" >/dev/null ||
  fail "maitri-theme-set-browser-policy installs color.json with install -T"
if grep -E 'mv -f' "$helper" >/dev/null; then
  fail "maitri-theme-set-browser-policy does not mv into a planted color.json directory"
fi
...
# STUB_GRANTED empty stands for an install whose maitri-settings predates the
# sudoers file. The default is granted, matching a current maitri.
...
if ((EUID == 0)); then
  ...
  [[ $elevation == "sudo /usr/bin/maitri-theme-set-browser-policy 1c2027" ]] ||
    fail "maitri-theme-set-browser-policy takes the passwordless sudo grant without a terminal" \
      "got: $elevation"

  dev_linked=$(MAITRI_PATH="$test_tmp/checkout" elevation_for 1c2027)
  [[ $dev_linked == "sudo /usr/bin/maitri-theme-set-browser-policy 1c2027" ]] ||
    fail "maitri-theme-set-browser-policy elevates the system install wherever MAITRI_PATH points" \
      "got: $dev_linked"

  pass "browser policy helper elevates a valid color through the sudo grant"

  ungranted=$(STUB_GRANTED="" elevation_for 1c2027)
  [[ $ungranted == "pkexec /usr/bin/maitri-theme-set-browser-policy 1c2027" ]] ||
    fail "maitri-theme-set-browser-policy falls back to polkit where the grant does not reach" \
      "got: $ungranted"
  ...
  for bad in "" "1C2027" "abc12" "abc1234" "1c202g" "../../etc/passwd" "1c2027 1c2027" \
    ...
    if PATH="$stub_bin:$PATH" ELEVATION_LOG="$test_tmp/elevation" \
      ...
      fail "maitri-theme-set-browser-policy rejects '$bad'"
    fi

    rejected=$(elevation_for "$bad")
    [[ -z $rejected ]] ||
      fail "maitri-theme-set-browser-policy rejects '$bad' before elevating" "got: $rejected"
  done

  if PATH="$stub_bin:$PATH" bash "$helper" 1c2027 ffffff </dev/null >/dev/null 2>&1; then
    fail "maitri-theme-set-browser-policy rejects more than one argument"
  fi
...
cat >"$setter_bin/maitri-theme-set-browser-policy" <<'SH'
#!/bin/bash
printf '%s\n' "$*" >"$COLOR_LOG"
SH
chmod +x "$setter_bin/maitri-theme-set-browser-policy"

cat >"$setter_bin/maitri-cmd-present" <<'SH'
#!/bin/bash
exit 1
SH
chmod +x "$setter_bin/maitri-cmd-present"

setter_home="$test_tmp/home"
theme_dir="$setter_home/.local/state/maitri/current/theme"
mkdir -p "$theme_dir"
...
color_for_theme() {
  ...
  HOME="$setter_home" COLOR_LOG="$test_tmp/color" PATH="$setter_bin:$stub_bin:$PATH" \
    MAITRI_PATH="$ROOT" bash "$setter" </dev/null >/dev/null 2>&1 || true
  cat "$test_tmp/color"
}

[[ $(color_for_theme "242,240,229") == "f2f0e5" ]] ||
  fail "maitri-theme-set-browser converts an RGB triple to six hex digits"
[[ $(color_for_theme $'14,31,41\n') == "0e1f29" ]] ||
  fail "maitri-theme-set-browser accepts a trailing newline"
[[ $(color_for_theme "0,0,0") == "000000" ]] ||
  fail "maitri-theme-set-browser pads single-digit components"
[[ $(color_for_theme " 12 , 11 , 12 ") == "0c0b0c" ]] ||
  fail "maitri-theme-set-browser tolerates surrounding whitespace"
...
for malformed in "" "not,a,color" "1,2" "1,2,3,4" "256,0,0" "999,999,999" "-1,0,0" \
  ...
  [[ $color == "1c2027" ]] ||
    fail "maitri-theme-set-browser falls back to the stock colour for '$malformed'" "got: $color"
done

[[ $(color_for_theme) == "1c2027" ]] ||
  fail "maitri-theme-set-browser falls back to the stock colour with no theme file"
