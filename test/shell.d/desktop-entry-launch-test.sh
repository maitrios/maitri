...
cat >"$mock_bin/maitri-font-set" <<'SH'
#!/bin/bash
printf 'font:%s\n' "$*" >>"$MAITRI_TEST_LOG"
SH

for command in maitri-pkg-aur-add maitri-install-emacs omazed maitri-theme-set-vscode maitri-install-gaming-gpu-lib32; do
...
#!/bin/bash
printf '%s\n' "$1" >"$MAITRI_TEST_PRESENTATION"
SH

chmod +x "$mock_bin"/*

export HOME="$test_home"
export MAITRI_TEST_LOG="$test_tmp/launch.log"
export MAITRI_TEST_PRESENTATION="$test_tmp/presentation"
...
wait_for_launch() {
  ...
  for ((attempt = 0; attempt < 100; attempt++)); do
    grep -Fxq "$expected" "$MAITRI_TEST_LOG" && return 0
    sleep 0.01
  done
...
assert_detached_installer_launch maitri-install-editor-emacs emacsclient
assert_detached_installer_launch maitri-install-editor-vscode code
assert_detached_installer_launch maitri-install-editor-zed dev.zed.Zed
assert_detached_installer_launch maitri-install-gaming-heroic heroic
assert_detached_installer_launch maitri-install-gaming-steam steam

bash "$ROOT/bin/maitri-install-and-launch" "Example App" "alpha beta" "Disk Usage"
presentation_command=$(<"$MAITRI_TEST_PRESENTATION")
...
[[ $presentation_command == *'maitri-pkg-add alpha beta && (setsid uwsm-app -- gtk-launch Disk\ Usage >/dev/null 2>&1 &)'* ]] ||
...
grep -Fxq 'pkg:alpha beta' "$MAITRI_TEST_LOG" ||
...
: >"$MAITRI_TEST_LOG"
if MAITRI_TEST_PKG_STATUS=1 bash -c "$presentation_command"; then
  fail "generic installer propagates package installation failure"
fi
if grep -q '^launch:' "$MAITRI_TEST_LOG"; then
...
run_presentation() {
  bash -c "sleep() { :; }; $(<"$MAITRI_TEST_PRESENTATION")"
}

bash "$ROOT/bin/maitri-install-app" "LM Studio" "lmstudio-bin"
presentation_command=$(<"$MAITRI_TEST_PRESENTATION")
[[ $presentation_command == *'echo Installing\ LM\ Studio...;'* ]] ||
  fail "install-app shell-quotes the display name" "$presentation_command"
[[ $presentation_command == *'maitri-pkg-add lmstudio-bin'* ]] ||
...
bash "$ROOT/bin/maitri-install-app" "Example App" "alpha beta"
presentation_command=$(<"$MAITRI_TEST_PRESENTATION")
[[ $presentation_command == 'echo Installing\ Example\ App...; maitri-pkg-add alpha beta' ]] ||
...
bash "$ROOT/bin/maitri-install-app" "Foo's App" "alpha"
presentation_command=$(<"$MAITRI_TEST_PRESENTATION")
...
grep -Fxq 'pkg:alpha' "$MAITRI_TEST_LOG" ||
...
bash "$ROOT/bin/maitri-install-app" "a'; echo PWNED; echo '" "alpha"
: >"$MAITRI_TEST_LOG"
...
bash "$ROOT/bin/maitri-install-font" "Cascadia Mono" "ttf-cascadia-mono-nerd" "CaskaydiaMono Nerd Font"
presentation_command=$(<"$MAITRI_TEST_PRESENTATION")
[[ $presentation_command == *'echo Installing\ Cascadia\ Mono...;'* ]] ||
  fail "install-font shell-quotes the display name" "$presentation_command"
[[ $presentation_command == *'maitri-font-set CaskaydiaMono\ Nerd\ Font'* ]] ||
  fail "install-font shell-quotes a font family with spaces" "$presentation_command"
: >"$MAITRI_TEST_LOG"
run_presentation
grep -Fxq 'pkg:ttf-cascadia-mono-nerd' "$MAITRI_TEST_LOG" ||
  fail "install-font installs the font package"
grep -Fxq 'font:CaskaydiaMono Nerd Font' "$MAITRI_TEST_LOG" ||
...
bash "$ROOT/bin/maitri-install-font" "a'; echo PWNED; echo '" "alpha" "a'; echo PWNED; echo '"
: >"$MAITRI_TEST_LOG"
...
bash "$ROOT/bin/maitri-install-app" "Example App" "alpha; echo PWNED"
: >"$MAITRI_TEST_LOG"
...
grep -Fxq 'pkg:alpha; echo PWNED' "$MAITRI_TEST_LOG" ||
  fail "install-app hands a hostile package list to the package helper as arguments" "$(<"$MAITRI_TEST_LOG")"
pass "install-app does not run extra commands from a package list"

bash "$ROOT/bin/maitri-install-app" "Example App" "$(printf 'alpha\nbeta')"
: >"$MAITRI_TEST_LOG"
run_presentation
grep -Fxq 'pkg:alpha beta' "$MAITRI_TEST_LOG" ||
  fail "install-app keeps every package when the list is newline-separated" "$(<"$MAITRI_TEST_LOG")"
pass "install-app keeps every package when the list is newline-separated"

env SHELLOPTS=errexit bash "$ROOT/bin/maitri-install-app" "Example App" "alpha beta" ||
  fail "install-app builds its command under an inherited errexit"
[[ $(<"$MAITRI_TEST_PRESENTATION") == 'echo Installing\ Example\ App...; maitri-pkg-add alpha beta' ]] ||
  fail "install-app builds the same command under an inherited errexit" "$(<"$MAITRI_TEST_PRESENTATION")"
env SHELLOPTS=errexit bash "$ROOT/bin/maitri-install-and-launch" "Example App" "alpha beta" "Disk Usage" ||
  fail "install-and-launch builds its command under an inherited errexit"
grep -Fq 'maitri-pkg-add alpha beta' "$MAITRI_TEST_PRESENTATION" ||
  fail "install-and-launch keeps its package list under an inherited errexit" "$(<"$MAITRI_TEST_PRESENTATION")"
pass "the installers build their command under an inherited errexit"

bash "$ROOT/bin/maitri-install-font" "Example Font" "alpha; echo PWNED" "Example Family"
: >"$MAITRI_TEST_LOG"
...
grep -Fxq 'pkg:alpha; echo PWNED' "$MAITRI_TEST_LOG" ||
  fail "install-font hands a hostile package to the package helper as one argument" "$(<"$MAITRI_TEST_LOG")"
pass "install-font does not run extra commands from its package"

bash "$ROOT/bin/maitri-install-font" "Example Font" "alpha" "Example Family"
: >"$MAITRI_TEST_LOG"
if MAITRI_TEST_PKG_STATUS=1 run_presentation; then
  fail "install-font propagates package installation failure"
fi
if grep -q '^font:' "$MAITRI_TEST_LOG"; then
  fail "install-font does not set the family after package installation failure" "$(<"$MAITRI_TEST_LOG")"
fi
pass "install-font does not set the family after package installation failure"

bash "$ROOT/bin/maitri-install-and-launch" "Example App" "alpha; echo PWNED" "Disk Usage"
: >"$MAITRI_TEST_LOG"
...
grep -Fxq 'pkg:alpha; echo PWNED' "$MAITRI_TEST_LOG" ||
  fail "install-and-launch hands a hostile package list to the package helper as arguments" "$(<"$MAITRI_TEST_LOG")"
