...
! grep -F 'systemd-networkd' "$dns" >/dev/null || fail "maitri-dns no longer restarts systemd-networkd"
grep -F 'NetworkManager/conf.d/20-maitri-dns.conf' "$dns" >/dev/null
...
if grep -F 'nmcli general reload conf,dns-full' "$dns" >/dev/null; then
  fail "maitri-dns must not push DNS before reapplying active profiles"
fi
pass "maitri-dns configures DNS through NetworkManager"
