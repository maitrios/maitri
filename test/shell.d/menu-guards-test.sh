...
const items = {
  'setup.default.browser.brave': { id: 'setup.default.browser.brave', when: 'maitri-pkg-present brave-bin', checked: '[[ "$(maitri-default-browser)" == "brave" ]]' },
  'setup.default.browser.zen': { id: 'setup.default.browser.zen', when: 'maitri-pkg-present zen-browser-bin', checked: '[[ "$(maitri-default-browser)" == "zen" ]]' },
  'plain': { id: 'plain', label: 'No guards' }
}
const script = menu.guardScript(items)
const browserSlot = `\${__maitri_read_${menu.guardReaders.indexOf('maitri-default-browser')}}`

assert(
  script.includes('if { maitri-pkg-present brave-bin; } >/dev/null 2>&1; then echo setup.default.browser.brave:w:1; else echo setup.default.browser.brave:w:0; fi'),
...
assertEqual(
  (script.match(/^__maitri_read_\d+=\$\(maitri-default-browser /gm) || []).length,
  1,
  'guard script reads a value command once for the whole batch'
)
assert(
  script.includes(`[[ "${browserSlot}" == "brave" ]]`) && !script.includes('"$(maitri-default-browser)"'),
  'guard script substitutes the captured answer into the expression'
)
assert(
  script.indexOf('__maitri_read_') < script.indexOf('if { maitri-pkg-present'),
...
const untouched = menu.guardScript({
  a: { id: 'a', when: 'command -v maitri-dns' },
  b: { id: 'b', when: '[[ "$(MAITRI_PATH=/usr/share/maitri maitri-channel-current)" == "stable" ]]' },
  c: { id: 'c', when: '(( $(maitri-default-browser | wc -l) == 1 ))' }
})
assert(
  untouched.includes('command -v maitri-dns')
    && untouched.includes('$(MAITRI_PATH=/usr/share/maitri maitri-channel-current)')
    && untouched.includes('$(maitri-default-browser | wc -l)'),
  'guard script leaves every form but the plain substitution to run the real command'
)
assert(
  !/^__maitri_read_/m.test(untouched),
...
const fs = require('fs')
const defaultItems = menu.parseMenuJsonc(fs.readFileSync(path.join(root, 'default/maitri/maitri-menu.jsonc'), 'utf8'))
const guardText = defaultItems.map(item => `${item.when}\n${item.checked}`).join('\n')
const repeated = [...new Set(
  (guardText.match(/\$\((maitri-[a-z0-9-]+)\)/g) || []).map(match => match.slice(2, -1))
...
for helper in maitri-pkg-present maitri-pkg-missing; do
...
for helper in maitri-cmd-present maitri-cmd-missing; do
  for case in "${cmd_cases[@]}"; do
    read -r -a argv <<<"$case"
    assert_helper_agrees "guard prelude resolves commands as the real helper does" "$helper" "${argv[@]}"
  done
done
pass "guard prelude resolves commands as maitri-cmd-present and maitri-cmd-missing do"
...
reader_script=$(node -e '
  ...
  process.stdout.write(menu.guardScript({
    hit: { id: "hit", checked: "[[ \"$(maitri-dns)\" == \"Cloudflare\" ]]" },
    miss: { id: "miss", checked: "[[ \"$(maitri-dns)\" == \"Google\" ]]" }
  }))
')
reader_result=$(bash -c '
maitri-dns() { printf "Cloudflare\n"; }
export -f maitri-dns
'"$reader_script")
...
# Update > Extra Themes runs maitri-theme-update, which pulls the themes that
...
# maitri-theme-extras today; the shapes below are what would tell us if one
# of them stopped.
themes_guard=$(node -e '
  const fs = require("fs")
  const path = require("path")
...
assert_themes_guard_agrees() {
  ...
  [[ -n $(HOME="$home" PATH="$ROOT/bin:$stub_dir:$PATH" "$ROOT/bin/maitri-theme-update" 2>/dev/null) ]] || updated=1
...
mkdir -p "$themes_home/empty/.config/maitri/themes"
mkdir -p "$themes_home/copied/.config/maitri/themes/handmade"
mkdir -p "$themes_home/cloned/.config/maitri/themes/tokyo-night/.git"
mkdir -p "$themes_home/linked/.config/maitri/themes" "$themes_home/checkout/.git"
ln -s "$themes_home/checkout" "$themes_home/linked/.config/maitri/themes/in-progress"
mkdir -p "$themes_home/worktree/.config/maitri/themes/branch"
printf 'gitdir: /elsewhere\n' >"$themes_home/worktree/.config/maitri/themes/branch/.git"

for shape in missing:1 empty:1 copied:1 cloned:0 linked:1 worktree:1; do
  assert_themes_guard_agrees \
    "Extra Themes shows exactly when maitri-theme-update has something to pull" \
    "$themes_home/${shape%:*}" "${shape#*:}"
done
pass "Extra Themes shows exactly when maitri-theme-update has something to pull"
...
# directory.
many="$themes_home/many/.config/maitri/themes"
mkdir -p "$many/tokyo night/.git" "$many/zen/.git" "$many/handmade"
ln -s "$themes_home/checkout" "$many/in-progress"

listed=$(HOME="$themes_home/many" LC_ALL=C "$ROOT/bin/maitri-theme-extras")
[[ $listed == "$many/tokyo night"$'\n'"$many/zen" ]] ||
  fail "maitri-theme-extras lists every clone and nothing else" "got: $listed"
pass "maitri-theme-extras lists every clone and nothing else"
...
HOME="$themes_home/many" LC_ALL=C GIT_CALLS="$git_calls" PATH="$ROOT/bin:$stub_dir:$PATH" \
  "$ROOT/bin/maitri-theme-update" >/dev/null 2>&1
pulled=$(<"$git_calls")
[[ $pulled == "<-C><$many/tokyo night><pull>"$'\n'"<-C><$many/zen><pull>" ]] ||
  fail "maitri-theme-update pulls each clone by its whole path" "got: $pulled"
pass "maitri-theme-update pulls each clone by its whole path"
