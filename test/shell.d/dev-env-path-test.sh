...
# Test against a copy so the test controls /etc/maitri.conf without mutating the host.
bootstrap="$tmpdir/env-bootstrap"
sed "s#/etc/maitri.conf#$tmpdir/maitri.conf#g" "$ROOT/default/bash/env-bootstrap" >"$bootstrap"

printf 'export MAITRI_PATH="/usr/share/maitri"\n' >"$tmpdir/maitri.conf"
mapfile -t default_result < <(run_bootstrap bash "$bootstrap" "$home" "$tmpdir/unrelated/bin:/usr/bin")
default_path=${default_result[1]}

[[ ${default_result[0]} == /usr/share/maitri ]] || fail "env-bootstrap resolves default MAITRI_PATH" "actual: ${default_result[0]}"
pass "env-bootstrap resolves default MAITRI_PATH"
...
printf 'export MAITRI_PATH="%s"\n' "$tmpdir/active" >"$tmpdir/maitri.conf"
mapfile -t linked_result < <(run_bootstrap bash "$bootstrap" "$home" "$tmpdir/unrelated/bin:/usr/bin")
linked_path=${linked_result[1]}

[[ ${linked_result[0]} == "$tmpdir/active" ]] || fail "env-bootstrap resolves linked MAITRI_PATH" "actual: ${linked_result[0]}"
pass "env-bootstrap resolves linked MAITRI_PATH"
