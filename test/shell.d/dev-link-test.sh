...
conf_file="$test_tmp/maitri.conf"
sudoers_file="$test_tmp/maitri-dev-path"
mkdir -p "$stub_bin" "$test_tmp/home"

cat >"$stub_bin/sudo" <<'SH'
#!/bin/bash

printf 'sudo' >>"$MAITRI_DEV_LINK_TEST_LOG"
for arg in "$@"; do
  printf '\t%s' "$arg" >>"$MAITRI_DEV_LINK_TEST_LOG"
done
printf '\n' >>"$MAITRI_DEV_LINK_TEST_LOG"

case "$1" in
  tee)
    cat >"$MAITRI_DEV_LINK_TEST_CONF"
    ;;
  install)
    # The staged file is the second-to-last argument.
    cp "${@: -2:1}" "$MAITRI_DEV_LINK_TEST_SUDOERS"
    ;;
esac
SH
...
printf 'gum' >>"$MAITRI_DEV_LINK_TEST_LOG"
for arg in "$@"; do
  printf '\t%s' "$arg" >>"$MAITRI_DEV_LINK_TEST_LOG"
done
printf '\n' >>"$MAITRI_DEV_LINK_TEST_LOG"
...
cat >"$stub_bin/maitri-system-reboot" <<'SH'
#!/bin/bash

printf 'reboot\n' >>"$MAITRI_DEV_LINK_TEST_LOG"
SH
chmod +x "$stub_bin/maitri-system-reboot"

run_link() {
  HOME="$test_tmp/home" \
    MAITRI_DEV_LINK_TEST_LOG="$log_file" \
    MAITRI_DEV_LINK_TEST_CONF="$conf_file" \
    MAITRI_DEV_LINK_TEST_SUDOERS="$sudoers_file" \
    PATH="$stub_bin:$PATH" \
    "$ROOT/bin/maitri-dev-link" "$@"
}
...
[[ $(<"$conf_file") == "export MAITRI_PATH=\"$checkout\"" ]] ||
  fail "dev link points MAITRI_PATH at the checkout" "$(<"$conf_file")"
pass "dev link points MAITRI_PATH at the checkout"

# sudo reads secure_path, not the caller's PATH, so the checkout has to come
# first there too or `sudo maitri-*` runs the packaged copy.
...
grep -Eq $'^sudo\tinstall\t-Dm440\t-o\troot\t-g\troot\t[^\t]+\t/etc/sudoers\\.d/maitri-dev-path$' "$log_file" ||
...
grep -F "sudo now resolves maitri-* from $checkout/bin" "$test_tmp/link.out" >/dev/null ||
