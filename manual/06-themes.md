...
maitri comes with twenty-two beautiful themes. You can select between them via _Style > Theme_ in the maitri Menu (`Super + Space`) or hop directly to the theme selector using `Super + Ctrl + Shift + Space`.

Each theme styles the desktop, terminal, neovim, activity screen (btop), Chromium, and the entire maitri shell: top bar, menu, notifications, OSD, and the lock screen. (For Obsidian, you must manually select the maitri theme via _Appearance > Themes_ inside the app).
