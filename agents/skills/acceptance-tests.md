...
Run acceptance tests in a disposable VM through the sibling `maitri-iso`
...
```bash
cd ../maitri-iso
./bin/maitri-iso-test release/<iso>.iso --reuse-base --sync-maitri ../maitri --no-preview
```

Use `--sync-all ../maitri` instead of `--sync-maitri ../maitri` when the
...
```bash
cd ../maitri-iso
./bin/maitri-iso-make --no-boot-offer --local-source ../maitri ../maitri-pkgs
./bin/maitri-iso-test release/<generated-iso>.iso --no-preview
```
