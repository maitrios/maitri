...
Panel {
  id: root
  moduleName: "maitri.network"
  ipcTarget: "maitri.network"
  ...
  // `maitri-network-status --verbose` samples (~1.5s apart via detailsPoll).
  ...
  // Wi-Fi band state from `maitri-network-band`. `bandCurrent` is the band
  ...
  // The share card is its own panel plugin (maitri.wifiqr) so a replacement
  ...
  function summonWifiQr(forceDetect) {
    ...
    bar.shell.summon("maitri.wifiqr", JSON.stringify(payload))
  }
  ...
  function refresh(scanWifi) {
    ...
    if (!bandProc.running) {
      bandProc.command = ["maitri-network-band"]
  ...
  function setBand(band) {
    ...
    actionProc.command = ["maitri-network-band", band]
    actionProc.running = true
  }

  // The speed test is its own panel plugin (maitri.speedtest) so a
  ...
  function summonSpeedTest() {
    ...
    bar.shell.summon("maitri.speedtest", connection ? JSON.stringify({ connection: connection }) : "{}")
  }

  function dnsCommand(provider) {
    var command = "maitri-dns"
  ...
  function setDns(provider) {
    ...
    if (provider === "Custom") {
      var launcher = "maitri-launch-floating-terminal-with-presentation"
  ...
  Process {
    id: detailsProc
    command: ["maitri-network-status", "--verbose"]
