# Tailscale maitri Widget

Native maitri bar widget for Tailscale.
...
`maitri-tailscale-receive` service, which announces each one with a
notification (an image preview when the file is an image, and a click to open
it). The Tailscale service install enables it; `maitri tailscale receive`
...
This widget ships as first-party plugin `maitri.tailscale`. Add it with `maitri plugin enable maitri.tailscale`, then place it with `maitri bar move maitri.tailscale` if desired.
