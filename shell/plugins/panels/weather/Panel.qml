...
Panel {
  ...
  // maitri-weather-location). The query is the wttr.in path segment
  ...
  property FileView locationFile: FileView {
    path: Quickshell.env("HOME") + "/.local/state/maitri/settings/weather.json"
    watchChanges: true
    printErrors: false
  ...
  function persistLocation(name, latitude, longitude) {
    if (name && latitude !== null && longitude !== null)
      locationSaveProc.command = ["maitri-weather-location", "--set", name, latitude + "," + longitude]
    else if (name)
      locationSaveProc.command = ["maitri-weather-location", "--set", name]
    else
      locationSaveProc.command = ["maitri-weather-location", "--clear"]
  ...
  // Mirrors maitri-weather-icon's wttr.in code → nerd-font glyph mapping.
