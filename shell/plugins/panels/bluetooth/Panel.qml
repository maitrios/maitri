...
Panel {
  id: root
  moduleName: "maitri.bluetooth"
  ipcTarget: "maitri.bluetooth"
  // manageIpc: false so this panel can own the single IpcHandler the target
  // permits — needed for the toggleBluetooth method below.
  manageIpc: false

  // Address -> "connecting" | "disconnecting" | "forgetting".
  // The actual Bluetooth sequencing lives in bin/maitri-bluetooth-device;
  ...
  function deviceCommand(action, address) {
    return ["maitri-bluetooth-device", action, address]
  }
  ...
  // disconnects, then removes the BlueZ pairing record via maitri-bluetooth-device.
  ...
  // the adapter came back on at the next boot. maitri-bluetooth-power moves the
  ...
  function toggleBluetooth() {
    if (!adapter) return
    Quickshell.execDetached(["maitri-bluetooth-power", adapter.enabled ? "off" : "on"])
  }

  IpcHandler {
    target: "maitri.bluetooth"
