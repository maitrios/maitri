...
`maitri-agent-usage-update` writes to `~/.local/state/maitri/agents/usage/`
...
`maitri plugin disable maitri.agents`.

## Data

Each agent is one JSON record in `~/.local/state/maitri/agents/usage/`,
written by `maitri-agent-usage-update`. That command runs one
`maitri-agent-usage-<agent>` collector per agent; the widget invokes it
...
configuration in `~/.config/maitri/agents/fireworks.json`:
...
- IPC: `maitri-shell maitri.agents <open|close|toggle|refresh|next>`.

## Settings

Settings live in the widget's entry in `~/.config/maitri/shell.json`. The
top-level keys can be set with
`maitri bar set maitri.agents <key> <value>`:
...
```bash
maitri bar set maitri.agents refreshIntervalSec 300 --json
maitri bar set maitri.agents syncDir '~/Sync/agent-usage'
```
...
```bash
maitri bar set maitri.agents providers '{
  "claude": { "enabled": true },
  "codex": { "enabled": false },
