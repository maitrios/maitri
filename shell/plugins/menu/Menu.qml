...
Item {
  ...
  // Injected by maitri-shell when this plugin is summoned.
  property string maitriPath: Quickshell.env("MAITRI_PATH")
  property var shell: null
  property var manifest: null

  // Plugin lifecycle hooks. The host calls open(payloadJson) after
  // `maitri-shell shell summon maitri.menu ...` and close() when hidden.
  ...
  property string defaultMenuPath: maitriPath + "/default/maitri/maitri-menu.jsonc"
  property string userMenuPath: Quickshell.env("HOME") + "/.config/maitri/extensions/maitri-menu.jsonc"
  ...
  readonly property var providers: ({
    "fonts": {
      script: "current=$(maitri-font-current 2>/dev/null); maitri-font-list 2>/dev/null | while read -r f; do [[ -z $f ]] && continue; printf '%s\\t%s\\t%s\\n' \"$f\" \"$f\" \"$current\"; done",
      icon: "",
      volatile: true,
      actionFor: function(value) { return "maitri-font-set " + Util.shellQuote(value) }
    },
    "power-profiles": {
      script: "current=$(powerprofilesctl get 2>/dev/null); maitri-powerprofiles-list 2>/dev/null | while read -r p; do [[ -z $p ]] && continue; printf '%s\\t%s\\t%s\\n' \"$p\" \"$p\" \"$current\"; done",
      icon: "\udb81\udc0b",
      actionFor: function(value) { return "maitri-powerprofiles-set autodetect " + Util.shellQuote(value) }
    }
  })
  ...
  // `maitri-shell shell summon maitri.menu '{"menu":"system"}'`.
  ...
  function openRoute(initialMenu) {
    ...
    // a leaf, e.g. `maitri menu summon screenrecord-stop`), run it directly
  ...
  // user extension at ~/.config/maitri/extensions/maitri-menu.jsonc) take
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "maitri-menu"
