...
// against `maitri-default-browser` -- so the batch runs it once and the rows
...
var GUARD_READERS = [
  "maitri-channel-current",
  "maitri-default-agent",
  "maitri-default-browser",
  "maitri-default-editor",
  "maitri-default-terminal",
  "maitri-dns"
]
...
function guardHelpers() {
  return 'declare -A __maitri_pkgs=()\n'
    + 'mapfile -t __maitri_pkg_names < <({ pacman -Qq; LC_ALL=C pacman -Qi'
    + " | awk '/^[A-Za-z]/ { provides = ($0 ~ /^Provides/); sub(/^[^:]*: /, \"\") }"
    + ' provides && $0 != "None" { n = split($0, p, " ");'
    + ' for (i = 1; i <= n; i++) { sub(/[<>=].*/, "", p[i]); print p[i] } }\'; } 2>/dev/null)\n'
    + 'for __maitri_pkg in "${__maitri_pkg_names[@]}"; do __maitri_pkgs[$__maitri_pkg]=1; done\n'
    + '__maitri_pkg_has() { [[ -n ${__maitri_pkgs[$1]-} ]] && return 0; '
    + '[[ $1 == *[\\<\\>=]* ]] && { pacman -Q "$1" &>/dev/null; return; }; return 1; }\n'
    + 'maitri-pkg-present() { local p; for p in "$@"; do __maitri_pkg_has "$p" || return 1; done; return 0; }\n'
    + 'maitri-pkg-missing() { local p; for p in "$@"; do __maitri_pkg_has "$p" || return 0; done; return 1; }\n'
    + 'maitri-cmd-present() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 1; done; return 0; }\n'
    + 'maitri-cmd-missing() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 0; done; return 1; }\n'
}
...
function guardPrelude(guards) {
  ...
  for (var i = 0; i < GUARD_READERS.length; i++) {
    ...
    prelude += "__maitri_read_" + i + "=$(" + GUARD_READERS[i] + " 2>/dev/null) || :\n"
  }
