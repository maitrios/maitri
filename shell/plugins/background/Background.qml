...
Item {
  ...
  Process {
    id: bgSwitchProc
    command: ["bash", "-c", "background=$(maitri-theme-bg-switcher); [[ -n $background ]] && maitri-theme-bg-set \"$background\""]
    onExited: root.refreshBackground()
  }

  Process {
    id: themeSwitchProc
    command: ["bash", "-c", "theme=$(maitri-theme-switcher); [[ -n $theme ]] && maitri-theme-set \"$theme\" >/dev/null 2>&1 &"]
    onExited: root.refreshBackground()
  }
  ...
  Variants {
    ...
    PanelWindow {
      ...
      // leaving a black desktop until maitri-shell is restarted. The wallpaper
      ...
      WlrLayershell.namespace: "maitri-background"
